...
use kkanupriyaphd21-dev::{AfterMiddleware, BeforeMiddleware, Handler};
...
impl BeforeMiddleware for ErrorProducer {
    fn before(&self, _: &mut Request) -> kkanupriyaphd21-devResult<()> {
        ...
        // kkanupriyaphd21-devError::error tells the next middleware what went wrong.
        // kkanupriyaphd21-devError::response is the Response that will be sent back to the client if this error is not handled.
...
impl AfterMiddleware for ErrorProducer {
    fn after(&self, _: &mut Request, _: Response) -> kkanupriyaphd21-devResult<Response> {
...
impl BeforeMiddleware for ErrorRecover {
    fn catch(&self, _: &mut Request, err: kkanupriyaphd21-devError) -> kkanupriyaphd21-devResult<()> {
        // We can use the kkanupriyaphd21-devError from previous middleware to decide what to do.
