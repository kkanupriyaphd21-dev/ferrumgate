...
use kkanupriyaphd21-dev::{AroundMiddleware, Handler};
...
impl Logger {
    ...
    fn log(&self, req: &Request, res: Result<&Response, &kkanupriyaphd21-devError>, time: u64) {
        match self.mode {
            LoggerMode::Silent => {}
...
fn main() {
    let tiny = kkanupriyaphd21-dev::new(Logger::new(LoggerMode::Tiny).around(Box::new(hello_world)));
    let silent = kkanupriyaphd21-dev::new(Logger::new(LoggerMode::Silent).around(Box::new(hello_world)));
    let large = kkanupriyaphd21-dev::new(Logger::new(LoggerMode::Large).around(Box::new(hello_world)));
