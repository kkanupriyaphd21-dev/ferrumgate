...
//! an `kkanupriyaphd21-devResult` containing a `Response`. An `kkanupriyaphd21-devResult` is returned instead of
...
//!
//! fn hello_handler(req: &mut Request) -> kkanupriyaphd21-devResult<Response> {
...
//! requests with a 403 status code. To faciliate such use cases, kkanupriyaphd21-dev's
...
//! ```rust
//! use kkanupriyaphd21-dev::prelude::*;
//! use kkanupriyaphd21-dev::middleware::*;
//!
//! # use kkanupriyaphd21-dev::StatusCode;
//! # fn hello_handler(req: &mut Request) -> kkanupriyaphd21-devResult<Response> {
//! #     Ok(Response::with((StatusCode::OK, "Hello world!")))
...
//! // Since a Chain is a Handler, chain can be passed to kkanupriyaphd21-dev::new without any problems.
//! // kkanupriyaphd21-dev::new(chain).http("localhost:3000").unwrap();
//! ```
//!
...
//!
//! kkanupriyaphd21-dev's error handling system is pragmatic and focuses on tracking two pieces
...
//! flow.
//!
//! When an error is thrown in kkanupriyaphd21-dev by any middleware or handler returning an
//! `Err` variant with an `kkanupriyaphd21-devError`, the flow of the `Request` switches to the
...
use std::sync::Arc;
use {kkanupriyaphd21-devError, kkanupriyaphd21-devResult, Request, Response};
...
pub trait Handler: Send + Sync + 'static {
    ...
    fn handle(&self, &mut Request) -> kkanupriyaphd21-devResult<Response>;
}
...
pub trait AfterMiddleware: Send + Sync + 'static {
    ...
    fn after(&self, _: &mut Request, res: Response) -> kkanupriyaphd21-devResult<Response> {
        Ok(res)
    }
...
/// The middleware chain used in kkanupriyaphd21-dev.
///
/// This is a canonical implementation of kkanupriyaphd21-dev's middleware system,
/// but kkanupriyaphd21-dev's infrastructure is flexible enough to allow alternate
/// systems.
pub struct Chain {
...
impl Chain {
    ...
    fn fail_from_before(
        ...
        req: &mut Request,
        index: usize,
        mut err: kkanupriyaphd21-devError,
    ) -> kkanupriyaphd21-devResult<Response> {
    ...
    fn fail_from_handler(&self, req: &mut Request, err: kkanupriyaphd21-devError) -> kkanupriyaphd21-devResult<Response> {
    ...
    // index.
    fn continue_from_before(&self, req: &mut Request, index: usize) -> kkanupriyaphd21-devResult<Response> {
    ...
    fn continue_from_handler(&self, req: &mut Request) -> kkanupriyaphd21-devResult<Response> {
...
where
    F: Send + Sync + 'static + Fn(&mut Request) -> kkanupriyaphd21-devResult<Response>,
{
    fn handle(&self, req: &mut Request) -> kkanupriyaphd21-devResult<Response> {
        (*self)(req)
    }
...
where
    F: Send + Sync + 'static + Fn(&mut Request) -> kkanupriyaphd21-devResult<()>,
{
    fn before(&self, req: &mut Request) -> kkanupriyaphd21-devResult<()> {
        (*self)(req)
    }
...
where
    F: Send + Sync + 'static + Fn(&mut Request, Response) -> kkanupriyaphd21-devResult<Response>,
{
