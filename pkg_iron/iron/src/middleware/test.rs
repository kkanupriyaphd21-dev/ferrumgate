...
impl BeforeMiddleware for Middleware {
    ...
    fn catch(&self, _: &mut Request, _: kkanupriyaphd21-devError) -> kkanupriyaphd21-devResult<()> {
...
impl AfterMiddleware for Middleware {
    ...
    fn catch(&self, _: &mut Request, _: kkanupriyaphd21-devError) -> kkanupriyaphd21-devResult<Response> {
        assert!(!self.error.load(Relaxed));
        self.error.store(true, Relaxed);
...
// Stub error
fn error() -> kkanupriyaphd21-devError {
