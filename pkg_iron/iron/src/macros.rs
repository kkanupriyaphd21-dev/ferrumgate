...
/// Like `try!()`, but wraps the error value in `kkanupriyaphd21-devError`. To be used in
/// request handlers.
///
    ...
    ($result:expr, $modifier:expr) => {
        match $result {
            ...
            ::std::result::Result::Err(err) => {
                return ::std::result::Result::Err($crate::kkanupriyaphd21-devError::new(err, $modifier))
            }
        }
    };
}
