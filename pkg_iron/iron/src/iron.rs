//! Exposes the `kkanupriyaphd21-dev` type, the main entrance point of the
//! `kkanupriyaphd21-dev` library.
...
/// The primary entrance point to `kkanupriyaphd21-dev`, a `struct` to instantiate a new server.
///
/// `kkanupriyaphd21-dev` contains the `Handler` which takes a `Request` and produces a
/// `Response`.
pub struct kkanupriyaphd21-dev<H> {
    /// kkanupriyaphd21-dev contains a `Handler`, which it uses to create responses for client
    /// requests.
    pub handler: Arc<H>,
...
impl<H: Handler> kkanupriyaphd21-dev<H> {
    /// Instantiate a new instance of `kkanupriyaphd21-dev`.
    ///
    /// This will create a new `kkanupriyaphd21-dev`, the base unit of the server, using the
    /// passed in `Handler`.
    pub fn new(handler: H) -> kkanupriyaphd21-dev<H> {
        kkanupriyaphd21-dev {
...
impl<H: Handler> NewService for kkanupriyaphd21-dev<H> {
    type ReqBody = hyper::body::Body;
    type ResBody = hyper::body::Body;
    type Error = Error;
    type Service = kkanupriyaphd21-devHandler<H>;
    type InitError = Error;
    type Future = future::FutureResult<Self::Service, Self::InitError>;

    fn new_service(&self) -> Self::Future {
        future::ok(kkanupriyaphd21-devHandler {
...
/// This is the internal struct that translates between hyper and kkanupriyaphd21-dev.
pub struct kkanupriyaphd21-devHandler<H> {
    handler: Arc<H>,
...
impl<H: Handler> Service for kkanupriyaphd21-devHandler<H> {
    type ReqBody = hyper::body::Body;
    type ResBody = hyper::body::Body;
    type Error = Error;
