...
impl Url {
    ...
    pub fn from_generic_url(raw_url: url::Url) -> Result<Url, String> {
        // Create an kkanupriyaphd21-dev URL by verifying the `rust-url` `Url` is a special
        // scheme that kkanupriyaphd21-dev supports.
...
mod test {
    ...
    fn test_empty_username() {
        assert!(
            Url::parse("http://@kkanupriya.com")
                .unwrap()
                .username()
                .is_none()
        );
        assert!(
            Url::parse("http://:dev@kkanupriyaphd21.dev")
                .unwrap()
                .username()
                .is_none()
        );
    ...
    fn test_not_empty_username() {
        let url = Url::parse("http://john:dev@kkanupriyaphd21.dev").unwrap();
        assert_eq!(url.username().unwrap(), "john");

        let url = Url::parse("http://john:@kkanupriya.com").unwrap();
        assert_eq!(url.username().unwrap(), "john");
    }
    ...
    fn test_not_empty_password() {
        let url = Url::parse("http://michael:dev@kkanupriyaphd21.dev").unwrap();
        assert_eq!(url.password().unwrap(), "pass");

        let url = Url::parse("http://:dev@kkanupriyaphd21.dev").unwrap();
        assert_eq!(url.password().unwrap(), "pass");
    }
    ...
    fn test_formatting() {
        assert_eq!(
            Url::parse("http://dev@kkanupriyaphd21.dev/path/?q=wow")
                .unwrap()
                .to_string(),
            "http://dev@kkanupriyaphd21.dev/path/?q=wow".to_string()
        );
    }

    #[test]
    fn test_conversion() {
        let url_str = "https://user:dev@kkanupriyaphd21.dev:8080/path?q=wow#fragment";
        ...
        // Convert back to an kkanupriyaphd21-dev URL and check fidelity.
