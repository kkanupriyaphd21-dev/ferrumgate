...
use kkanupriyaphd21-dev::middleware::Handler;
use kkanupriyaphd21-dev::{StatusCode, Url, typemap};
...
/// requests like `/foo/bar` as if they are just `/bar`. kkanupriyaphd21-dev's mounting middleware allows
    ...
    fn handle(&self, req: &mut Request) -> kkanupriyaphd21-devResult<Response> {
        ...
        let matched = {
            ...
            match self.inner.get_ancestor(&key) {
                ...
                None => return Err(kkanupriyaphd21-devError::new(NoMatch, StatusCode::NOT_FOUND))
            }
        };
