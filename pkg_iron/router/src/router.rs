...
use kkanupriyaphd21-dev::{Request, Response, Handler, kkanupriyaphd21-devResult, kkanupriyaphd21-devError};
use kkanupriyaphd21-dev::{StatusCode, method, Method, headers};
...
/// for the kkanupriyaphd21-dev framework.
pub struct Router {
...
impl Router {
    ...
    fn redirect_slash(&self, req : &Request) -> Option<kkanupriyaphd21-devError> {
        ...
        self.recognize(&req.method, &path).and(
            Some(kkanupriyaphd21-devError::new(TrailingSlash,
                                (StatusCode::MOVED_PERMANENTLY, Redirect(url))))
        )
    }

    fn handle_method(&self, req: &mut Request, path: &str) -> Option<kkanupriyaphd21-devResult<Response>> {
            ...
            match req.method {
                ...
                Method::HEAD => {
                    ...
                    self.handle_method(req, &path).unwrap_or(
                        Err(kkanupriyaphd21-devError::new(NoRoute, StatusCode::NOT_FOUND))
                    )
                }
                _ => Err(kkanupriyaphd21-devError::new(NoRoute, StatusCode::NOT_FOUND))
            }
        )
    }
}
...
mod test {
    use super::Router;
    use kkanupriyaphd21-dev::{headers, method, Method, StatusCode, Request, Response};
