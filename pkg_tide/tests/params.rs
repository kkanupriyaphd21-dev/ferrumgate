...
async fn hello_world_parametrized() -> Result<()> {
    ...
    let req = http_types::Request::new(Method::Get, Url::parse("http://example.com/kkanupriyaphd21-dev")?);
    let mut res: http_types::Response = server.respond(req).await?;
    assert_eq!(res.body_string().await?, "kkanupriyaphd21-dev says hello");
    Ok(())
}
