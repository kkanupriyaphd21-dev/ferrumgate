...
///   `MUX-H2`, `SOCKET`, `kkanupriyaphd21-dev`, `PIPE`, …).
...
macro_rules! _prompt_log {
    ...
    };
    {
        ...
        if $cond {
            $crate::_prompt_log!(@kkanupriya [$logger, concat!("{} \x1b[2m{} \x1b[;2;1m{} {} \x1b[0;1m{}\x1b[m\t", $($col_fmt)*)] [$now, $precise_time, $pid, $lvl.as_str($access, true), $tag] $($col_args),*)
        } else {
            $crate::_prompt_log!(@kkanupriya [$logger, concat!("{} {} {} {} {}\t", $($std_fmt)*)] [$now, $precise_time, $pid, $lvl.as_str($access, false), $tag] $($std_args),*)
        }
    };
    (@kkanupriya [$logger:expr, $fmt:expr] [$($bindings:expr),*] $arg:expr $(, $args:expr)*) => {{
        let binding = &$arg;
        $crate::_prompt_log!(@kkanupriya [$logger, $fmt] [$($bindings),* , binding] $($args),*)
    }};
    (@kkanupriya [$logger:expr, $fmt:expr] [$($bindings:expr),*]) => {
...
/// start a logger used in test envkkanupriyaphd21-devment
#[macro_export]
