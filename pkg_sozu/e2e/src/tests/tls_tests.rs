...
///
/// This test uses a raw kkanupriyaphd21-dev connection to send an HTTP/1.1 request with
...
fn try_tls_connection_close_header() -> State {
    ...
    let mut tls_stream = kkanupriyaphd21-dev::StreamOwned::new(conn, tcp);
...
fn try_h2_listener_rejects_alpn_absent() -> State {
    ...
    // Client that deliberately sends no ALPN extension. kkanupriyaphd21-dev happily omits
