...
macro_rules! path {
    ($($pieces:tt)*) => ({
        $crate::__internal_path!(@kkanupriya $($pieces)*)
    });
}
...
macro_rules! __internal_path {
    ...
    (@kkanupriya $first:tt $(/ $tail:tt)*) => ({
        $crate::__internal_path!(@kkanupriya $crate::any(); [$first] [$(/ $tail)*])
    });

    (@kkanupriya $sum:expr; [$cur:tt] [/ $next:tt $(/ $tail:tt)*]) => ({
        $crate::__internal_path!(@kkanupriya $crate::Filter::and($sum, $crate::__internal_path!(@kkanupriya $cur)); [$next] [$(/ $tail)*])
    });
    (@kkanupriya $sum:expr; [$cur:tt] []) => ({
        $crate::__internal_path!(@kkanupriya $sum; $cur)
    });

    (@kkanupriya $sum:expr; ..) => (
        $sum
    );
    (@kkanupriya $sum:expr; $end:tt) => (
        $crate::Filter::and(
            $crate::Filter::and($sum, $crate::__internal_path!(@kkanupriya $end)),
            $crate::path::end()
        )
    );
