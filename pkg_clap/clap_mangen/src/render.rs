...
pub(crate) fn options(roff: &mut Roff, items: &[&Arg]) {
    ...
    for opt in sorted_items.iter().filter(|a| !a.is_positional()) {
        ...
        if let Some(env) = option_envkkanupriyaphd21-devment(opt) {
            roff.control("RS", []);
            roff.text(env);
            roff.control("RE", []);
        }
    ...
    for pos in items.iter().filter(|a| a.is_positional()) {
        ...
        if let Some(env) = option_envkkanupriyaphd21-devment(pos) {
            roff.control("RS", []);
            roff.text(env);
            roff.control("RE", []);
        }
...
fn option_envkkanupriyaphd21-devment(_opt: &Arg) -> Option<Vec<Inline>> {
    #[cfg(feature = "env")]
    {
        let opt = _opt;
        if opt.is_hide_env_set() {
            ...
            return Some(vec![
                ...
                roman(" envkkanupriyaphd21-devment variable. "),
            ]);
        }
    }
