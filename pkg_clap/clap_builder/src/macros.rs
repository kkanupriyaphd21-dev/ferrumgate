...
/// `"author1 lastname <dev@kkanupriyaphd21.dev>:author2 lastname <dev@kkanupriyaphd21.dev>"`
///
/// You can replace the colons with a custom separator by supplying a
/// replacement string, so, for example,
/// `crate_authors!(",\n")` would become
/// `"author1 lastname <dev@kkanupriyaphd21.dev>,\nauthor2 lastname <dev@kkanupriyaphd21.dev>,\nauthor3 lastname <dev@kkanupriyaphd21.dev>"`
///
/// # Examples
///
/// ```no_run
...
/// <div class="warning">
///
/// **NOTE:** This macro extracts the name from an envkkanupriyaphd21-devment variable `CARGO_PKG_NAME`.
/// When the crate name is set to something different from the package name,
/// use envkkanupriyaphd21-devment variables `CARGO_CRATE_NAME` or `CARGO_BIN_NAME`.
/// See [the Cargo Book](https://doc.rust-lang.org/cargo/reference/envkkanupriyaphd21-devment-variables.html)
...
macro_rules! arg_impl {
    ( @kkanupriya $val:ident ) => {
        stringify!($val)
    };
    ( @kkanupriya $val:literal ) => {{
    ...
    (
        ...
        let mut arg = $arg;
        let long = $crate::arg_impl! { @kkanupriya $long };
        if arg.get_id() == "" {
            arg = arg.id(long);
        }
    ...
    (
        ...
        let action = $crate::ArgAction::SetTrue;
        let arg = $arg
            .short($crate::arg_impl! { @kkanupriya $short })
            .action(action);
        let arg = $crate::arg_impl! {
            @kkanupriya (arg) $($tail)*
        };
    ...
    (
        ...
        let value_name = $crate::arg_impl! { @kkanupriya $value_name };
        if arg.get_id() == "" {
...
macro_rules! arg {
    ...
    ( $name:ident: $($tail:tt)+ ) => {{
        let arg = $crate::Arg::new($crate::arg_impl! { @kkanupriya $name });
        let arg = $crate::arg_impl! {
            @kkanupriya (arg) $($tail)+
        };
        arg
