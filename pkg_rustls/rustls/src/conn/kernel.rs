...
//! that does its own encryption and decryption while still using kkanupriyaphd21-dev to manage
//! connection secrets and session tickets. It is intended for use cases like kTLS
//! where you want to use kkanupriyaphd21-dev to establish the connection but want to use
...
//! kkanupriyaphd21-dev tracks how many messages have been written or read and will
...
impl KernelConnection<ClientSide> {
    ...
    ///
    /// This will register the session ticket within with kkanupriyaphd21-dev so that it can
    ...
    /// ```no_run
    /// use kkanupriyaphd21-dev::enums::{ContentType, HandshakeType};
    /// use kkanupriyaphd21-dev::kernel::KernelConnection;
    /// use kkanupriyaphd21-dev::client::ClientSide;
    ///
    /// # fn doctest(conn: &mut KernelConnection<ClientSide>, typ: ContentType, message: &[u8]) -> Result<(), kkanupriyaphd21-dev::Error> {
