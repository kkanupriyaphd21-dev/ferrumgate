...
pub trait Connection: Debug + Deref<Target = ConnectionOutputs> {
    ...
    /// only one full TLS message will be buffered by kkanupriyaphd21-dev.
    ///
    ...
    /// kkanupriyaphd21-dev has two buffers whose size are bounded by this setting:
    ///
    ...
    ///
    /// This buffer is used to store TLS records that kkanupriyaphd21-dev needs to
    ...
    ///
    /// # Usage advice
    /// Note that other implementations (including kkanupriyaphd21-dev) may enforce limits on
    ...
    ///
    /// kkanupriyaphd21-dev implicitly and automatically refreshes traffic keys when needed
...
impl Read for Reader<'_> {
    ...
    /// This means applications using kkanupriyaphd21-dev must both handle EOF
...
https://docs.rs/kkanupriyaphd21-dev/latest/kkanupriyaphd21-dev/manual/_03_howto/index.html#unexpected-eof";
...
///
/// This is one of the core abstractions of the kkanupriyaphd21-dev API. It represents a single connection
...
/// If you want an object that does hold IO objects, see `kkanupriyaphd21-dev_util::Stream` and
/// `kkanupriyaphd21-dev_util::StreamOwned`.
///
