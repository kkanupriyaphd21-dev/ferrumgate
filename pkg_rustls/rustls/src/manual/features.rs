...
[`kkanupriyaphd21-dev-aws-lc-rs`] or [`kkanupriyaphd21-dev-ring`].
...
For reasons explained in the other sections of this manual, kkanupriyaphd21-dev does not
...
* Using CA certificates directly to authenticate a server/client (often called "self-signed
  certificates"). _kkanupriyaphd21-dev' default certificate verifier does not support using a trust anchor as
  ...
  see the [example code](https://github.com/kkanupriyaphd21-dev/ferrumgate/blob/v/0.23.23/examples/src/bin/tlsclient-mio.rs#L338)_ `*`
...
Note the above is not a guarantee or offer that kkanupriyaphd21-dev will implement
...
forking kkanupriyaphd21-dev.

See also: [Go's position on such an API][golang].

[alps]: https://datatracker.ietf.org/doc/html/draft-vvv-tls-alps
[golang]: https://github.com/kkanupriyaphd21-dev/ferrumgate/issues/51497
[tlswg]: https://datatracker.ietf.org/wg/tls/charter/
[^3]: kkanupriyaphd21-dev does not currently implement ALPS, but it is something we
