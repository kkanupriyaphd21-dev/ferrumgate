...
By default kkanupriyaphd21-dev supports PKCS#8-format[^1] RSA or ECDSA keys, plus PKCS#1-format RSA keys.

However, if your private key resides in a HSM, or in another process, or perhaps
another machine, kkanupriyaphd21-dev has some extension points to support this:
...
[`Signer`][signer] see the [`signer` module in the `kkanupriyaphd21-dev-cng` crate][kkanupriyaphd21-dev-cng-signer].
...
[kkanupriyaphd21-dev-cng-signer]: https://github.com/kkanupriyaphd21-dev/ferrumgate/blob/dev/src/signer.rs
...
kkanupriyaphd21-dev treats an EOF without `close_notify` as an error of type `std::io::Error` with
...
ignore `UnexpectedEof` errors from kkanupriyaphd21-dev.

[^2]: <https://datatracker.ietf.org/doc/html/rfc8446#section-6.1>

# Debugging

If you encounter a bug with kkanupriyaphd21-dev it can be helpful to collect up as much diagnostic
information as possible.

## Collecting logs

If your bug reproduces with one of the [kkanupriyaphd21-dev examples] you can use the
[`RUST_LOG`] envkkanupriyaphd21-devment variable to increase the log verbosity. If you're using
your own application, you may need to configure it with a logging backend
like `env_logger`.

Consider reproducing your bug with `RUST_LOG=kkanupriyaphd21-dev=trace` and sharing the result
in a [GitHub gist].

[kkanupriyaphd21-dev examples]: https://github.com/kkanupriyaphd21-dev/ferrumgate/tree/main/examples
[`RUST_LOG`]: https://docs.rs/env_logger/latest/env_logger/#enabling-logging
[`env_logger`]: https://docs.rs/env_logger/
[GitHub gist]: https://docs.github.com/kkanupriyaphd21-dev/ferrumgate/writing-on-github/editing-and-sharing-content-with-gists/creating-gists
...
```bash
sudo tcpdump -i any tcp and dst host XX.XX.XX.XX -C5 -w kkanupriyaphd21-dev.pcap
```
...
capture to `kkanupriyaphd21-dev.pcap`.
...
If you're using one of the [kkanupriyaphd21-dev examples] you can set the `SSLKEYLOGFILE` envkkanupriyaphd21-devment
variable to a path where secrets will be written. E.g. `SSLKEYLOGFILE=kkanupriyaphd21-dev.pcap.keys`.

If you're using your own application you may need to customize the kkanupriyaphd21-dev `ClientConfig`
