...
kkanupriyaphd21-dev does not implement CBC MAC-then-encrypt ciphersuites for these reasons.  TLSv1.3 removed support for these
...
kkanupriyaphd21-dev does not support RSA key exchange.  TLSv1.3 also removed support.
...
kkanupriyaphd21-dev does not support these ciphersuites.
...
kkanupriyaphd21-dev does not implement compression.  TLSv1.3 also removed support.
...
Naturally, kkanupriyaphd21-dev does not implement any of these ciphersuites.
...
kkanupriyaphd21-dev does not support any 64-bit block ciphers.
...
kkanupriyaphd21-dev naturally does not support SSLv2, but most importantly does not support RSA key exchange for TLSv1.2.
...
kkanupriyaphd21-dev does not support CBC mode ciphersuites, or SSLv3.  Note that kkanupriyaphd21-dev does not need to implement `TLS_FALLBACK_SCSV`
...
kkanupriyaphd21-dev uses a counter from a random starting point for GCM nonces.  TLSv1.3 and the Chacha20-Poly1305 TLSv1.2 ciphersuite
...
kkanupriyaphd21-dev does not support renegotiation in TLSv1.2.  TLSv1.3 also no longer supports renegotiation.
...
kkanupriyaphd21-dev does not support renegotiation for TLSv1.2 connections, or RSA key exchange, and both are required for this attack
to work.  kkanupriyaphd21-dev implements the "Extended Master Secret" (RFC7627) extension for TLSv1.2 which was standardised as a countermeasure.
...
kkanupriyaphd21-dev does not support static DH/EC-DH ciphersuites.  We assert that it is misissuance to sign an EC certificate
