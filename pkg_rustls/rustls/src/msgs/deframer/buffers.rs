...
pub trait TlsInputBuffer {
    ...
    /// incrementally over multiple calls into kkanupriyaphd21-dev.  As a result the
    ...
    ///
    /// kkanupriyaphd21-dev guarantees it will not `discard()` more bytes than are returned
    /// from `slice_mut()`.
    fn discard(&mut self, num_bytes: usize);
}
