...
///
/// # Background
/// kkanupriyaphd21-dev always sends a presumptive key share in its `ClientHello`, using
...
///
/// # How it works
/// This function is only called by kkanupriyaphd21-dev for clients.  It is called when
/// constructing the initial `ClientHello`.  kkanupriyaphd21-dev follows these steps:
///
...
///    we start again.
/// 2. The server agrees to our hybrid key exchange: kkanupriyaphd21-dev calls
///    [`ActiveKeyExchange::complete()`] consuming `self`.
/// 3. The server agrees to our classical key exchange: kkanupriyaphd21-dev calls
...
enum_builder! {
    ...
    /// kkanupriyaphd21-dev supports. The supported groups are determined via the
