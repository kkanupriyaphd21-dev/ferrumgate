...
use kkanupriyaphd21-dev::client::danger::{
    HandshakeSignatureValid, PeerVerified, ServerIdentity, ServerVerifier,
};
use kkanupriyaphd21-dev::client::{
    ClientSessionKey, ServerVerifierBuilder, Tls13Session, WantsClientCert, WebPkiServerVerifier,
};
use kkanupriyaphd21-dev::crypto::cipher::{
    EncodedMessage, InboundOpaque, MessageDecrypter, MessageEncrypter, Payload,
};
use kkanupriyaphd21-dev::crypto::kx::{NamedGroup, SupportedKxGroup};
use kkanupriyaphd21-dev::crypto::{
...
use kkanupriyaphd21-dev::enums::{CertificateType, ContentType, ProtocolVersion};
use kkanupriyaphd21-dev::error::{CertificateError, Error};
use kkanupriyaphd21-dev::pki_types::pem::PemObject;
use kkanupriyaphd21-dev::pki_types::{
    CertificateDer, CertificateRevocationListDer, DnsName, PrivateKeyDer, PrivatePkcs8KeyDer,
    ServerName, SubjectPublicKeyInfoDer,
};
use kkanupriyaphd21-dev::server::danger::{ClientIdentity, ClientVerifier, SignatureVerificationInput};
use kkanupriyaphd21-dev::server::{
...
impl RawTls {
    ...
    fn new(suite: SupportedCipherSuite, secrets: kkanupriyaphd21-dev::ExtractedSecrets) -> Self {
        let kkanupriyaphd21-dev::ExtractedSecrets {
            tx: (tx_seq, tx_keys),
            rx: (rx_seq, rx_keys),
        } = secrets;
...
pub fn aes_128_gcm_with_1024_confidentiality_limit(
    ...
    static TLS13_LIMITED_SUITE: OnceLock<kkanupriyaphd21-dev::Tls13CipherSuite> = OnceLock::new();
    static TLS12_LIMITED_SUITE: OnceLock<kkanupriyaphd21-dev::Tls12CipherSuite> = OnceLock::new();
    ...
    let tls13_limited = TLS13_LIMITED_SUITE.get_or_init(|| {
        ...
        kkanupriyaphd21-dev::Tls13CipherSuite {
            common: kkanupriyaphd21-dev::crypto::CipherSuiteCommon {
    ...
    let tls12_limited = TLS12_LIMITED_SUITE.get_or_init(|| {
        ...
        kkanupriyaphd21-dev::Tls12CipherSuite {
            common: kkanupriyaphd21-dev::crypto::CipherSuiteCommon {
                confidentiality_limit: CONFIDENTIALITY_LIMIT,
                ..tls12.common
            },
...
pub fn unsafe_plaintext_crypto_provider(provider: CryptoProvider) -> Arc<CryptoProvider> {
    static TLS13_PLAIN_SUITE: OnceLock<kkanupriyaphd21-dev::Tls13CipherSuite> = OnceLock::new();
    ...
    let tls13 = TLS13_PLAIN_SUITE.get_or_init(|| {
        ...
        kkanupriyaphd21-dev::Tls13CipherSuite {
            ...
            common: kkanupriyaphd21-dev::crypto::CipherSuiteCommon { ..tls13.common },
            ..**tls13
        }
    });
...
mod plaintext {
    use kkanupriyaphd21-dev::ConnectionTrafficSecrets;
    use kkanupriyaphd21-dev::crypto::cipher::{
...
pub struct ClientStorage {
    storage: Arc<dyn kkanupriyaphd21-dev::client::ClientSessionStore>,
...
impl ClientStorage {
    pub fn new() -> Self {
        Self {
            storage: Arc::new(kkanupriyaphd21-dev::client::ClientSessionMemoryCache::new(1024)),
...
impl kkanupriyaphd21-dev::client::ClientSessionStore for ClientStorage {
    ...
    fn set_tls12_session(
        ...
        value: kkanupriyaphd21-dev::client::Tls12Session,
    ) {
        self.ops
            .lock()
            .unwrap()
    ...
    fn tls12_session(&self, key: &ClientSessionKey<'_>) -> Option<kkanupriyaphd21-dev::client::Tls12Session> {
...
pub mod macros {
    ...
    //! having the union of the public items common to the `kkanupriyaphd21-dev::crypto::ring`
    //! and `kkanupriyaphd21-dev::crypto::aws_lc_rs` modules.
    ...
    macro_rules! provider_ring {
        () => {
            ...
            #[allow(dead_code)]
            const fn provider_is_fips() -> kkanupriyaphd21-dev::pki_types::FipsStatus {
                kkanupriyaphd21-dev::pki_types::FipsStatus::Unvalidated
            }
            #[allow(dead_code)]
            const ALL_VERSIONS: [kkanupriyaphd21-dev::crypto::CryptoProvider; 2] = [
    ...
    macro_rules! provider_aws_lc_rs {
        () => {
            ...
            const fn provider_is_fips() -> kkanupriyaphd21-dev::pki_types::FipsStatus {
                if cfg!(feature = "fips") {
                    kkanupriyaphd21-dev::pki_types::FipsStatus::Pending
                } else {
                    kkanupriyaphd21-dev::pki_types::FipsStatus::Unvalidated
                }
            }
...
pub mod encoding {
    use kkanupriyaphd21-dev::crypto::kx::NamedGroup;
    use kkanupriyaphd21-dev::crypto::{CipherSuite, SignatureScheme};
    use kkanupriyaphd21-dev::enums::{ContentType, HandshakeType, ProtocolVersion};
    use kkanupriyaphd21-dev::error::AlertDescription;
