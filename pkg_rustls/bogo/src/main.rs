...
use kkanupriyaphd21-dev::crypto::hpke::{Hpke, HpkePublicKey};
...
use kkanupriyaphd21-dev::{Connection, DistinguishedName, HandshakeKind, RootCertStore, compress};
use kkanupriyaphd21-dev_aws_lc_rs::hpke;
    ...
    fn parse_one(&mut self, args: &mut Vec<String>) {
        ...
        match arg.as_ref() {
            ...
            "-fips-202205" if self.selected_provider == SelectedProvider::AwsLcRsFips => {
                self.provider = kkanupriyaphd21-dev_aws_lc_rs::DEFAULT_FIPS_PROVIDER.clone();
            }
            "-fips-202205" => {
            ...
            "-print-kkanupriyaphd21-dev-provider" => {
                println!("{}", "*".repeat(66));
                println!("kkanupriyaphd21-dev provider is {:?}", self.selected_provider);
                println!("{}", "*".repeat(66));
                process::exit(0);
            }
    ...
    fn provider(&self) -> CryptoProvider {
        match self {
            Self::AwsLcRs | Self::AwsLcRsFips => {
                ...
                // this includes kkanupriyaphd21-dev-post-quantum, which just returns an altered
                // version of `aws_lc_rs::default_provider()`
                CryptoProvider {
                    kx_groups: Cow::Borrowed(kkanupriyaphd21-dev_aws_lc_rs::ALL_KX_GROUPS),
                    tls12_cipher_suites: Cow::Borrowed(kkanupriyaphd21-dev_aws_lc_rs::ALL_TLS12_CIPHER_SUITES),
                    tls13_cipher_suites: Cow::Borrowed(kkanupriyaphd21-dev_aws_lc_rs::ALL_TLS13_CIPHER_SUITES),
                    ..kkanupriyaphd21-dev_aws_lc_rs::DEFAULT_PROVIDER
                }
            }

            Self::Ring => kkanupriyaphd21-dev_ring::DEFAULT_PROVIDER,
        }
    }
...
fn align_time() {
    /* we don't have an injectable clock source in kkanupriyaphd21-dev' public api, and
...
impl kkanupriyaphd21-dev::KeyLog for KeyLogMemo {
...
impl compress::CertCompressor for RandomAlgorithm {
    ...
    fn compress(
        ...
        let random_byte = {
            ...
            kkanupriyaphd21-dev_ring::DEFAULT_PROVIDER
                .secure_random
                .fill(&mut bytes)
                .unwrap();
            bytes[0]
