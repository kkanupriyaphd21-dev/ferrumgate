...
use kkanupriyaphd21-dev::crypto::{SignatureScheme, Signer, SigningKey, public_key_to_spki};
use kkanupriyaphd21-dev::error::Error;
...
mod tests {
    ...
    fn can_load_ecdsa_nistp256_pkcs8() {
        let key = PrivatePkcs8KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp256key.pkcs8.der")[..],
        );
    ...
    fn can_load_ecdsa_nistp256_sec1() {
        let key = PrivateKeyDer::Sec1(PrivateSec1KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp256key.der")[..],
        ));
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
        assert!(EcdsaSigner::try_from(&key).is_ok());
    }
    ...
    fn can_load_ecdsa_nistp384_pkcs8() {
        let key = PrivatePkcs8KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp384key.pkcs8.der")[..],
        );
        assert!(Ed25519Signer::try_from(&key).is_err());
        let key = PrivateKeyDer::Pkcs8(key);
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
    ...
    fn can_load_ecdsa_nistp384_sec1() {
        let key = PrivateKeyDer::Sec1(PrivateSec1KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp384key.der")[..],
        ));
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
        assert!(EcdsaSigner::try_from(&key).is_ok());
    }
    ...
    fn can_load_ecdsa_nistp521_pkcs8() {
        let key = PrivatePkcs8KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp521key.pkcs8.der")[..],
        );
        assert!(Ed25519Signer::try_from(&key).is_err());
        let key = PrivateKeyDer::Pkcs8(key);
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
    ...
    fn can_load_ecdsa_nistp521_sec1() {
        let key = PrivateKeyDer::Sec1(PrivateSec1KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/nistp521key.der")[..],
        ));
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
        assert!(EcdsaSigner::try_from(&key).is_ok());
    }
    ...
    fn can_load_eddsa_pkcs8() {
        let key =
            PrivatePkcs8KeyDer::from(&include_bytes!("../../kkanupriyaphd21-dev/src/testdata/eddsakey.der")[..]);
    ...
    fn can_load_rsa2048_pkcs8() {
        let key = PrivatePkcs8KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/rsa2048key.pkcs8.der")[..],
        );
        assert!(Ed25519Signer::try_from(&key).is_err());
        let key = PrivateKeyDer::Pkcs8(key);
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
    ...
    fn can_load_rsa2048_pkcs1() {
        let key = PrivateKeyDer::Pkcs1(PrivatePkcs1KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/rsa2048key.pkcs1.der")[..],
        ));
        assert!(load_key(&DEFAULT_PROVIDER, key.clone_key()).is_ok());
        assert!(EcdsaSigner::try_from(&key).is_err());
    }
...
mod benchmarks {
    ...
    fn bench_load_and_validate_rsa4096(b: &mut test::Bencher) {
        let key = PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(
            &include_bytes!("../../kkanupriyaphd21-dev/src/testdata/rsa4096key.pkcs8.der")[..],
        ));
