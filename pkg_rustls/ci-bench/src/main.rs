...
use kkanupriyaphd21-dev::crypto::{CipherSuite, CryptoProvider, GetRandomFailed, SecureRandom, TicketProducer};
use kkanupriyaphd21-dev::enums::ProtocolVersion;
use kkanupriyaphd21-dev::server::{NoServerSessionStorage, ServerSessionMemoryCache, WebPkiClientVerifier};
use kkanupriyaphd21-dev::{
    ClientConfig, ClientConnection, Connection, HandshakeKind, RootCertStore, ServerConfig,
    ServerConnection,
};
use kkanupriyaphd21-dev_test::KeyType;
...
/// [`kkanupriyaphd21-dev::client::ClientSessionMemoryCache`] and [`kkanupriyaphd21-dev::server::ServerSessionMemoryCache`],
...
fn all_benchmarks_params() -> Vec<BenchmarkParams> {
    ...
    for (provider, ticketer, provider_name, warm_up) in [
        (
            derandomize(kkanupriyaphd21-dev_ring::DEFAULT_PROVIDER),
            &(ring_ticketer as fn() -> Arc<dyn TicketProducer>),
            "ring",
            None,
        ),
        (
            derandomize(kkanupriyaphd21-dev_aws_lc_rs::DEFAULT_PROVIDER),
    ...
    let make_ticketer =
        &((|| Arc::new(kkanupriyaphd21-dev_fuzzing_provider::Ticketer)) as fn() -> Arc<dyn TicketProducer>);

    all.push(BenchmarkParams::new(
        kkanupriyaphd21-dev_fuzzing_provider::PROVIDER_TLS13.into(),
        make_ticketer,
    ...
    all.push(BenchmarkParams::new(
        kkanupriyaphd21-dev_fuzzing_provider::PROVIDER_TLS12.into(),
        make_ticketer,
        AuthKeySource::FuzzingProvider,
...
impl ServerSideStepper<'_> {
    fn make_config(params: &BenchmarkParams, resume: ResumptionKind) -> Arc<ServerConfig> {
        ...
        let mut cfg = match params.auth_key {
            ...
            AuthKeySource::FuzzingProvider => cfg
                .with_client_cert_verifier(WebPkiClientVerifier::no_client_auth())
                .with_server_credential_resolver(kkanupriyaphd21-dev_fuzzing_provider::server_cert_resolver())
                .unwrap(),
        };
