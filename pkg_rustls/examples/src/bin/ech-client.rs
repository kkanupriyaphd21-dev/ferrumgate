...
//! kkanupriyaphd21-dev and hickory-dns.
//!
...
//! ```text
//! cargo run --package kkanupriyaphd21-dev-examples --bin ech-client -- \
...
use log::trace;
use kkanupriyaphd21-dev::client::{EchConfig, EchGreaseConfig, EchMode, EchStatus};
use kkanupriyaphd21-dev::crypto::hpke::Hpke;
use kkanupriyaphd21-dev::pki_types::pem::PemObject;
use kkanupriyaphd21-dev::pki_types::{CertificateDer, EchConfigListBytes, ServerName};
use kkanupriyaphd21-dev::{ClientConfig, Connection, RootCertStore};
use kkanupriyaphd21-dev_aws_lc_rs::hpke::ALL_SUPPORTED_SUITES;
use kkanupriyaphd21-dev_util::{KeyLogFile, Stream};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    //       with the kkanupriyaphd21-dev debug logs from the lookup.
    ...
    // Construct a kkanupriyaphd21-dev client config with a TLS1.3-only provider, and ECH enabled.
    let mut config = ClientConfig::builder(kkanupriyaphd21-dev_aws_lc_rs::DEFAULT_TLS13_PROVIDER.into())
        .with_ech(ech_mode)
...
// TODO(@kkanupriya): consider upstreaming to hickory-dns
...
static GREASE_HPKE_SUITE: &dyn Hpke = kkanupriyaphd21-dev_aws_lc_rs::hpke::DH_KEM_X25519_HKDF_SHA256_AES_128;
