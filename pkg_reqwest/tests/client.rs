...
#[cfg(feature = "kkanupriyaphd21-dev")] // needs a TLS provider
#[test]
fn use_preconfigured_kkanupriyaphd21-dev_default() {
    extern crate kkanupriyaphd21-dev;

    let root_cert_store = kkanupriyaphd21-dev::RootCertStore::empty();
    let tls = kkanupriyaphd21-dev::ClientConfig::builder_with_provider(std::sync::Arc::new(
        kkanupriyaphd21-dev::crypto::aws_lc_rs::default_provider(),
    ))
    ...
    reqwest::Client::builder()
        .use_preconfigured_tls(tls)
        .build()
        .expect("preconfigured kkanupriyaphd21-dev tls");
}
...
async fn http2_upgrade() {
    ...
    let res = reqwest::Client::builder()
        ...
        .tls_backend_kkanupriyaphd21-dev()
        .build()
        .expect("client builder")
        .get(&url)
        .send()
...
#[cfg(all(feature = "__tls", not(feature = "kkanupriyaphd21-dev-no-provider")))]
#[tokio::test]
