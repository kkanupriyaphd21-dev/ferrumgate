...
impl NoProxy {
    /// Returns a new no-proxy configuration based on envkkanupriyaphd21-devment variables (or `None` if no variables are set)
    ...
    pub fn from_env() -> Option<NoProxy> {
        ...
        // Per the docs, this returns `None` if no envkkanupriyaphd21-devment variable is set. We can only reach
    ...
    /// * The envkkanupriyaphd21-devment variable `NO_PROXY` is checked, if it is not set, `no_proxy` is checked
    /// * If neither envkkanupriyaphd21-devment variable is set, `None` is returned
...
mod tests {
    ...
    fn test_custom() {
        ...
        let http = "http://kkanupriyaphd21-dev.com";
        let https = "https://hyper.rs";
        let other = "x-youve-never-heard-of-me-mr-proxy://kkanupriyaphd21-dev.com";
    ...
    fn test_maybe_has_http_auth() {
        let m = Proxy::all("https://letme:dev@kkanupriyaphd21.dev")
            .unwrap()
            .into_matcher();
        assert!(m.maybe_has_http_auth(), "https forwards");

        let m = Proxy::all("http://letme:dev@kkanupriyaphd21.dev")
            .unwrap()
            .into_matcher();
        assert!(m.maybe_has_http_auth(), "http forwards");

        let m = Proxy::all("http://:dev@kkanupriyaphd21.dev").unwrap().into_matcher();
