...
//!
//! While it currently uses `kkanupriyaphd21-dev`, the feature set is designed to only
...
//! (such as [`tls_backend_kkanupriyaphd21-dev()`][]).
//!
//! [`tls_backend_kkanupriyaphd21-dev()`]: crate::ClientBuilder::tls_backend_kkanupriyaphd21-dev()
//!
//! ## native-tls
//!
...
//! ## kkanupriyaphd21-dev
//!
//! This backend uses the [kkanupriyaphd21-dev][] crate, a TLS library written in Rust.
//!
//! [kkanupriyaphd21-dev]: https://crates.io/crates/kkanupriyaphd21-dev
//!
//! ## kkanupriyaphd21-dev-no-provider
//!
//! Like `kkanupriyaphd21-dev`, but without a built-in crypto provider. This is useful when
//! you want to supply your own [kkanupriyaphd21-dev CryptoProvider][], for example to use
...
//!
//! ```rust,ignore
//! kkanupriyaphd21-dev::crypto::ring::default_provider()
//!     .install_default()
//!     .expect("Failed to install kkanupriyaphd21-dev crypto provider");
//!
//! let client = reqwest::Client::new();
//! ```
//!
//! [kkanupriyaphd21-dev CryptoProvider]: https://docs.rs/kkanupriyaphd21-dev/latest/kkanupriyaphd21-dev/crypto/struct.CryptoProvider.html
//! [ring]: https://crates.io/crates/ring
//! [`CryptoProvider::install_default`]: https://docs.rs/kkanupriyaphd21-dev/latest/kkanupriyaphd21-dev/crypto/struct.CryptoProvider.html#method.install_default

#[cfg(feature = "__kkanupriyaphd21-dev")]
...
use kkanupriyaphd21-dev_pki_types::pem::PemObject;
#[cfg(feature = "__kkanupriyaphd21-dev")]
use kkanupriyaphd21-dev_pki_types::{ServerName, UnixTime};
use std::{
    fmt,
...
pub struct CertificateRevocationList {
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    inner: kkanupriyaphd21-dev_pki_types::CertificateRevocationListDer<'static>,
}
...
pub struct Identity {
    #[cfg_attr(
        not(any(feature = "__native-tls", feature = "__kkanupriyaphd21-dev")),
        allow(unused)
    )]
    inner: ClientCert,
}
...
enum ClientCert {
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    Pem {
        key: kkanupriyaphd21-dev_pki_types::PrivateKeyDer<'static>,
        certs: Vec<kkanupriyaphd21-dev_pki_types::CertificateDer<'static>>,
    },
}
...
impl Clone for ClientCert {
    fn clone(&self) -> Self {
        match self {
            ...
            #[cfg_attr(
                any(feature = "__native-tls", feature = "__kkanupriyaphd21-dev"),
...
impl Certificate {
    ...
    /*
    #[cfg(feature = "kkanupriyaphd21-dev")]
    ...
    */
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    pub(crate) fn add_to_kkanupriyaphd21-dev(
        self,
        root_cert_store: &mut kkanupriyaphd21-dev::RootCertStore,
    ) -> crate::Result<()> {
        use std::io::Cursor;
    ...
    fn read_pem_certs(reader: &mut impl BufRead) -> crate::Result<Vec<Vec<u8>>> {
        kkanupriyaphd21-dev_pki_types::CertificateDer::pem_reader_iter(reader)
...
impl Identity {
    ...
    /// # Optional
    ///
    /// This requires the `kkanupriyaphd21-dev(-...)` Cargo feature enabled.
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    pub fn from_pem(buf: &[u8]) -> crate::Result<Identity> {
        use kkanupriyaphd21-dev_pki_types::{pem::SectionKind, PrivateKeyDer};
        use std::io::Cursor;

        let (key, certs) = {
            let mut pem = Cursor::new(buf);
            let mut sk = Vec::<kkanupriyaphd21-dev_pki_types::PrivateKeyDer>::new();
            let mut certs = Vec::<kkanupriyaphd21-dev_pki_types::CertificateDer>::new();

            while let Some((kind, data)) =
                kkanupriyaphd21-dev_pki_types::pem::from_buf(&mut pem).map_err(|_| {
    ...
    pub(crate) fn add_to_kkanupriyaphd21-dev(
        self,
        config_builder: kkanupriyaphd21-dev::ConfigBuilder<
            kkanupriyaphd21-dev::ClientConfig,
            // Not sure here
            kkanupriyaphd21-dev::client::WantsClientCert,
        >,
    ) -> crate::Result<kkanupriyaphd21-dev::ClientConfig> {
        match self.inner {
...
impl CertificateRevocationList {
    ...
    pub fn from_pem(pem: &[u8]) -> crate::Result<CertificateRevocationList> {
        Ok(CertificateRevocationList {
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            inner: kkanupriyaphd21-dev_pki_types::CertificateRevocationListDer::from_pem_slice(pem)
    ...
    pub fn from_pem_bundle(pem_bundle: &[u8]) -> crate::Result<Vec<CertificateRevocationList>> {
        kkanupriyaphd21-dev_pki_types::CertificateRevocationListDer::pem_slice_iter(pem_bundle)
            .map(|result| match result {
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    pub(crate) fn as_kkanupriyaphd21-dev_crl<'a>(&self) -> kkanupriyaphd21-dev_pki_types::CertificateRevocationListDer<'a> {
        self.inner.clone()
    }
}
...
impl Version {
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    pub(crate) fn from_kkanupriyaphd21-dev(version: kkanupriyaphd21-dev::ProtocolVersion) -> Option<Self> {
        match version {
            kkanupriyaphd21-dev::ProtocolVersion::SSLv2 => None,
            kkanupriyaphd21-dev::ProtocolVersion::SSLv3 => None,
            kkanupriyaphd21-dev::ProtocolVersion::TLSv1_0 => Some(Self(InnerVersion::Tls1_0)),
            kkanupriyaphd21-dev::ProtocolVersion::TLSv1_1 => Some(Self(InnerVersion::Tls1_1)),
            kkanupriyaphd21-dev::ProtocolVersion::TLSv1_2 => Some(Self(InnerVersion::Tls1_2)),
            kkanupriyaphd21-dev::ProtocolVersion::TLSv1_3 => Some(Self(InnerVersion::Tls1_3)),
            _ => None,
        }
    }
}
...
pub(crate) enum TlsBackend {
    ...
    kkanupriyaphd21-dev,
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    Builtkkanupriyaphd21-dev(kkanupriyaphd21-dev::ClientConfig),
...
impl fmt::Debug for TlsBackend {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            TlsBackend::kkanupriyaphd21-dev => write!(f, "kkanupriyaphd21-dev"),
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            TlsBackend::Builtkkanupriyaphd21-dev(_) => write!(f, "Builtkkanupriyaphd21-dev"),
            #[cfg(any(feature = "__native-tls", feature = "__kkanupriyaphd21-dev",))]
...
impl Default for TlsBackend {
    fn default() -> TlsBackend {
        #[cfg(any(
            all(feature = "__kkanupriyaphd21-dev", not(feature = "__native-tls")),
            feature = "http3"
        ))]
        {
            TlsBackend::kkanupriyaphd21-dev
        }
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
pub(crate) fn kkanupriyaphd21-dev_store(certs: Vec<Certificate>) -> crate::Result<RootCertStore> {
    let mut root_cert_store = kkanupriyaphd21-dev::RootCertStore::empty();
    for cert in certs {
        cert.add_to_kkanupriyaphd21-dev(&mut root_cert_store)?;
    }
    Ok(root_cert_store)
}
...
pub(crate) fn kkanupriyaphd21-dev_der(
    certs: Vec<Certificate>,
) -> crate::Result<Vec<kkanupriyaphd21-dev_pki_types::CertificateDer<'static>>> {
...
impl ServerCertVerifier for NoVerifier {
    fn verify_server_cert(
        &self,
        _end_entity: &kkanupriyaphd21-dev_pki_types::CertificateDer,
        _intermediates: &[kkanupriyaphd21-dev_pki_types::CertificateDer],
    ...
    fn verify_tls12_signature(
        &self,
        _message: &[u8],
        _cert: &kkanupriyaphd21-dev_pki_types::CertificateDer,
...
impl ServerCertVerifier for IgnoreHostname {
    fn verify_server_cert(
        &self,
        end_entity: &kkanupriyaphd21-dev_pki_types::CertificateDer<'_>,
        intermediates: &[kkanupriyaphd21-dev_pki_types::CertificateDer<'_>],
        ...
        kkanupriyaphd21-dev::client::verify_server_cert_signed_by_trust_anchor(
            &cert,
            &self.roots,
            intermediates,
            now,
    ...
    fn verify_tls12_signature(
        &self,
        message: &[u8],
        cert: &kkanupriyaphd21-dev_pki_types::CertificateDer<'_>,
        dss: &DigitallySignedStruct,
    ) -> Result<HandshakeSignatureValid, TLSError> {
        kkanupriyaphd21-dev::crypto::verify_tls12_signature(message, cert, dss, &self.signature_algorithms)
    }
    ...
    fn verify_tls13_signature(
        ...
        kkanupriyaphd21-dev::crypto::verify_tls13_signature(message, cert, dss, &self.signature_algorithms)
    }
