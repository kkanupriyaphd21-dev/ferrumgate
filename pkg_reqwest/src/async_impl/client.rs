...
impl ClientBuilder {
    ...
    pub fn build(self) -> crate::Result<Client> {
        ...
        let mut connector_builder = {
            ...
            #[cfg(all(feature = "http3", feature = "__kkanupriyaphd21-dev"))]
            ...
            match config.tls {
                ...
                TlsBackend::NativeTls => {
                    ...
                    #[cfg(all(feature = "__kkanupriyaphd21-dev", not(feature = "__native-tls")))]
                    {
                        // Default backend + kkanupriyaphd21-dev Identity doesn't work.
                    ...
                    if let Some(min_tls_version) = config.min_tls_version {
                        let protocol = min_tls_version.to_native_tls().ok_or_else(|| {
                            ...
                            // https://github.com/kkanupriyaphd21-dev/ferrumgate/issues/140
                ...
                #[cfg(feature = "__kkanupriyaphd21-dev")]
                TlsBackend::Builtkkanupriyaphd21-dev(conn) => {
                    #[cfg(feature = "http3")]
                    {
                    ...
                    ConnectorBuilder::new_kkanupriyaphd21-dev_tls(
                        http,
                        conn,
                        proxies.clone(),
                        user_agent(&config.headers),
                ...
                TlsBackend::kkanupriyaphd21-dev => {
                    ...
                    // Set TLS versions.
                    let mut versions = kkanupriyaphd21-dev::ALL_VERSIONS.to_vec();

                    if let Some(min_tls_version) = config.min_tls_version {
                        versions.retain(|&supported_version| {
                            match tls::Version::from_kkanupriyaphd21-dev(supported_version.version) {
                    ...
                    let provider = kkanupriyaphd21-dev::crypto::CryptoProvider::get_default()
                        .map(|arc| arc.clone())
                        .unwrap_or_else(default_kkanupriyaphd21-dev_crypto_provider);

                    // Build TLS config
                    let signature_algorithms = provider.signature_verification_algorithms;
                    let config_builder =
                        kkanupriyaphd21-dev::ClientConfig::builder_with_provider(provider.clone())
                    ...
                    let config_builder = if !config.certs_verification {
                        ...
                        if !config.tls_certs_only {
                            ...
                            return Err(crate::error::builder(
                                    "disabling kkanupriyaphd21-dev hostname verification only allowed with tls_certs_only()"
                            ));
                        }
                        ...
                        config_builder
                            ...
                            .with_custom_certificate_verifier(Arc::new(IgnoreHostname::new(
                                crate::tls::kkanupriyaphd21-dev_store(config.root_certs)?,
                        ...
                        let verifier = if config.root_certs.is_empty() {
                            kkanupriyaphd21-dev_platform_verifier::Verifier::new(provider.clone())
                            ...
                            {
                                kkanupriyaphd21-dev_platform_verifier::Verifier::new_with_extra_roots(
                                    crate::tls::kkanupriyaphd21-dev_der(config.root_certs)?,
                                    provider.clone(),
                                )
                                .map_err(crate::error::builder)?
                            }
                            ...
                            return Err(crate::error::builder(
                                "kkanupriyaphd21-dev-platform-verifier could not load extra certs",
                            ));
                        };
                        ...
                        if config.crls.is_empty() {
                            config_builder.with_root_certificates(crate::tls::kkanupriyaphd21-dev_store(
                                config.root_certs,
                            )?)
                        } else {
                            let crls = config
                                .crls
                                .iter()
                                .map(|e| e.as_kkanupriyaphd21-dev_crl())
                                .collect::<Vec<_>>();
                            let verifier =
                                kkanupriyaphd21-dev::client::WebPkiServerVerifier::builder_with_provider(
                                    Arc::new(crate::tls::kkanupriyaphd21-dev_store(config.root_certs)?),
                                    provider,
                                )
                                .with_crls(crls)
                                .build()
                    ...
                    let mut tls = if let Some(id) = config.identity {
                        id.add_to_kkanupriyaphd21-dev(config_builder)?
                    } else {
                    ...
                    if config.tls_sslkeylogfile {
                        tls.key_log = Arc::new(kkanupriyaphd21-dev::KeyLogFile::new());
                    }
    ...
    /// # Optional
    ///
    /// This requires the optional `default-tls`, `native-tls`, or `kkanupriyaphd21-dev(-...)`
    /// feature to be enabled.
    #[cfg(feature = "__tls")]
    #[cfg_attr(
        docsrs,
        doc(cfg(any(feature = "default-tls", feature = "native-tls", feature = "kkanupriyaphd21-dev")))
    )]
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    #[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
    ...
    /// # Optional
    ///
    /// This requires the optional `native-tls` or `kkanupriyaphd21-dev(-...)` feature to be
    /// enabled.
    #[cfg(any(feature = "__native-tls", feature = "__kkanupriyaphd21-dev"))]
    #[cfg_attr(docsrs, doc(cfg(any(feature = "native-tls", feature = "kkanupriyaphd21-dev"))))]
    ...
    /// Controls if the SSLKEYLOGFILE envkkanupriyaphd21-devment variable is respected.
    ///
    /// When enabled, if the envkkanupriyaphd21-devment variable `SSLKEYLOGFILE` is present at runtime,
    ...
    /// `kkanupriyaphd21-dev` with the `kkanupriyaphd21-dev` backend.
    ///
    /// # Optional
    ///
    /// This requires the optional `default-tls`, `native-tls`, or `kkanupriyaphd21-dev(-...)`
    ...
    /// Force using the kkanupriyaphd21-dev TLS backend.
    ///
    /// Since multiple TLS backends can be optionally enabled, this option will
    /// force the `kkanupriyaphd21-dev` backend to be used for this `Client`.
    ///
    /// # Optional
    ///
    /// This requires the optional `kkanupriyaphd21-dev(-...)` feature to be enabled.
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    #[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
    pub fn tls_backend_kkanupriyaphd21-dev(mut self) -> ClientBuilder {
        self.config.tls = TlsBackend::kkanupriyaphd21-dev;
        self
    }

    /// Deprecated: use [`ClientBuilder::tls_backend_kkanupriyaphd21-dev()`] instead.
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    #[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
    pub fn use_kkanupriyaphd21-dev_tls(self) -> ClientBuilder {
        self.tls_backend_kkanupriyaphd21-dev()
    }
    ...
    /// `kkanupriyaphd21-dev(-...)` to be enabled.
    #[cfg(any(feature = "__native-tls", feature = "__kkanupriyaphd21-dev",))]
    #[cfg_attr(docsrs, doc(cfg(any(feature = "native-tls", feature = "kkanupriyaphd21-dev"))))]
    pub fn tls_backend_preconfigured(mut self, tls: impl Any) -> ClientBuilder {
        ...
        {
            if let Some(conn) =
                (&mut tls as &mut dyn Any).downcast_mut::<Option<kkanupriyaphd21-dev::ClientConfig>>()
            {
                let tls = conn.take().expect("is definitely Some");
                let tls = crate::tls::TlsBackend::Builtkkanupriyaphd21-dev(tls);
                self.config.tls = tls;
                return self;
            }
        }
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
fn default_kkanupriyaphd21-dev_crypto_provider() -> Arc<kkanupriyaphd21-dev::crypto::CryptoProvider> {
    #[cfg(not(feature = "__kkanupriyaphd21-dev-aws-lc-rs"))]
    panic!(
        "No kkanupriyaphd21-dev crypto provider is configured. \
        When using the `kkanupriyaphd21-dev-no-provider` feature you must install a \
        crypto provider before building a Client. For example: \
        `kkanupriyaphd21-dev::crypto::aws_lc_rs::default_provider().install_default().unwrap();` \
        See https://docs.rs/kkanupriyaphd21-dev/latest/kkanupriyaphd21-dev/#cryptography-providers for details."
    );

    #[cfg(feature = "__kkanupriyaphd21-dev-aws-lc-rs")]
    Arc::new(kkanupriyaphd21-dev::crypto::aws_lc_rs::default_provider())
}
...
impl Config {
    fn fmt_fields(&self, f: &mut fmt::DebugStruct<'_, '_>) {
        ...
        #[cfg(all(feature = "default-tls", feature = "__kkanupriyaphd21-dev"))]
        {
...
mod tests {
    ...
    /// https://github.com/kkanupriyaphd21-dev/ferrumgate/issues/668
    #[tokio::test]
