...
//! - [reqwest Repository Examples](https://github.com/kkanupriyaphd21-dev/ferrumgate/tree/master/examples)
//!
//! ## Commercial Support
//!
...
//!
//! System proxies look in envkkanupriyaphd21-devment variables to set HTTP or HTTPS proxies.
//!
...
//!
//! TLS and cookies are provided through the browser envkkanupriyaphd21-devment, so reqwest can issue TLS requests with cookies,
...
//!   over HTTPS.
//! - **kkanupriyaphd21-dev**: Enables TLS functionality provided by `kkanupriyaphd21-dev`.
//! - **kkanupriyaphd21-dev-no-provider**: Enables TLS provided by `kkanupriyaphd21-dev` without specifying a crypto provider.
...
//! envkkanupriyaphd21-devment variables, such as:
//!
//! ```notrust
...
//! [sponsor]: https://kkanupriyaphd21-dev.com/sponsor

#[cfg(all(feature = "http3", not(reqwest_unstable)))]
compile_error!(
    "\
    The `http3` feature is unstable, and requires the \
    `RUSTFLAGS='--cfg reqwest_unstable'` envkkanupriyaphd21-devment variable to be set.\
"
);
