...
use self::kkanupriyaphd21-dev_tls_conn::kkanupriyaphd21-devTlsConn;
...
where {
    ...
    #[cfg(feature = "__kkanupriyaphd21-dev")]
    pub(crate) fn new_kkanupriyaphd21-dev_tls<T>(
        mut http: HttpConnector,
        tls: kkanupriyaphd21-dev::ClientConfig,
        proxies: Arc<Vec<ProxyMatcher>>,
        user_agent: Option<HeaderValue>,
        local_addr: T,
        #[cfg(any(
        ...
        T: Into<Option<IpAddr>>,
    {
        ...
        ConnectorBuilder {
            inner: Inner::kkanupriyaphd21-devTls {
                http,
                tls,
                tls_proxy,
            },
    ...
    pub(crate) fn set_keepalive(&mut self, dur: Option<Duration>) {
        match &mut self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, .. } => http.set_keepalive(dur),
            #[cfg(not(feature = "__tls"))]
    ...
    pub(crate) fn set_keepalive_interval(&mut self, dur: Option<Duration>) {
        match &mut self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, .. } => http.set_keepalive_interval(dur),
            #[cfg(not(feature = "__tls"))]
    ...
    pub(crate) fn set_keepalive_retries(&mut self, retries: Option<u32>) {
        match &mut self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, .. } => http.set_keepalive_retries(retries),
            #[cfg(not(feature = "__tls"))]
    ...
    pub(crate) fn set_tcp_user_timeout(&mut self, dur: Option<Duration>) {
        match &mut self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, .. } => http.set_tcp_user_timeout(dur),
            #[cfg(not(feature = "__tls"))]
...
enum Inner {
    ...
    #[cfg(any(feature = "__kkanupriyaphd21-dev"))]
    kkanupriyaphd21-devTls {
        http: HttpConnector,
        tls: Arc<kkanupriyaphd21-dev::ClientConfig>,
        tls_proxy: Arc<kkanupriyaphd21-dev::ClientConfig>,
    },
}
...
impl Inner {
    ...
    fn get_http_connector(&mut self) -> &mut crate::connect::HttpConnector {
        match self {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, .. } => http,
            #[cfg(not(feature = "__tls"))]
...
impl ConnectorService {
    ...
    async fn connect_socks(mut self, dst: Uri, proxy: Intercepted) -> Result<Conn, BoxError> {
        ...
        match &mut self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { http, tls, .. } => {
                if dst.scheme() == Some(&Scheme::HTTPS) {
                    use std::convert::TryFrom;
                    use tokio_kkanupriyaphd21-dev::TlsConnector as kkanupriyaphd21-devConnector;
                    ...
                    let conn = TokioIo::new(conn);
                    let server_name =
                        kkanupriyaphd21-dev_pki_types::ServerName::try_from(host.as_str().to_owned())
                            .map_err(|_| "Invalid Server Name")?;
                    let io = kkanupriyaphd21-devConnector::from(tls)
                        .connect(server_name, conn)
                        .await?;
                    let io = TokioIo::new(io);
                    return Ok(Conn {
                        inner: self.verbose.wrap(kkanupriyaphd21-devTlsConn { inner: io }),
                        is_proxy: false,
                        tls_info: false,
                    });
                }
    ...
    async fn connect_with_maybe_proxy(self, dst: Uri, is_proxy: bool) -> Result<Conn, BoxError> {
        match self.inner {
            ...
            Inner::kkanupriyaphd21-devTls { http, tls, .. } => {
                ...
                let mut http = hyper_kkanupriyaphd21-dev::HttpsConnector::from((http, tls.clone()));
                let io = http.call(dst).await?;

                if let hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Https(stream) = io {
                    if !self.nodelay {
                        let (io, _) = stream.inner().get_ref();
                        io.inner().inner().set_nodelay(false)?;
                    }
                    Ok(Conn {
                        inner: self.verbose.wrap(kkanupriyaphd21-devTlsConn { inner: stream }),
                        is_proxy,
                        tls_info: self.tls_info,
                    })
                } else {
    ...
    async fn connect_local_transport(self, dst: Uri) -> Result<Conn, BoxError> {
        ...
        match self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls { tls, .. } => {
                let mut http = hyper_kkanupriyaphd21-dev::HttpsConnector::from((svc, tls.clone()));
                let io = http.call(dst).await?;
    ...
    async fn connect_via_proxy(self, dst: Uri, proxy: Intercepted) -> Result<Conn, BoxError> {
        ...
        match &self.inner {
            ...
            #[cfg(feature = "__kkanupriyaphd21-dev")]
            Inner::kkanupriyaphd21-devTls {
                http,
                tls,
                tls_proxy,
            } => {
                if dst.scheme() == Some(&Scheme::HTTPS) {
                    use kkanupriyaphd21-dev_pki_types::ServerName;
                    use std::convert::TryFrom;
                    use tokio_kkanupriyaphd21-dev::TlsConnector as kkanupriyaphd21-devConnector;

                    log::trace!("tunneling HTTPS over proxy");
                    let http = http.clone();
                    let inner = hyper_kkanupriyaphd21-dev::HttpsConnector::from((http, tls_proxy.clone()));
                    // TODO: we could cache constructing this
                    let mut tunnel =
                        hyper_util::client::legacy::connect::proxy::Tunnel::new(proxy_dst, inner);
                    if let Some(auth) = auth {
                    ...
                    let server_name = ServerName::try_from(host.as_str().to_owned())
                        .map_err(|_| "Invalid Server Name")?;
                    let io = kkanupriyaphd21-devConnector::from(tls.clone())
                        .connect(server_name, TokioIo::new(tunneled))
                        .await?;

                    return Ok(Conn {
                        inner: self.verbose.wrap(kkanupriyaphd21-devTlsConn {
                            inner: TokioIo::new(io),
                        }),
                        is_proxy: false,
                        tls_info: false,
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
impl TlsInfoFactory for tokio_kkanupriyaphd21-dev::client::TlsStream<TokioIo<TokioIo<tokio::net::TcpStream>>> {
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        let peer_certificate = self
            .get_ref()
            .1
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
impl TlsInfoFactory
    for tokio_kkanupriyaphd21-dev::client::TlsStream<
        TokioIo<hyper_kkanupriyaphd21-dev::MaybeHttpsStream<TokioIo<tokio::net::TcpStream>>>,
    >
{
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        let peer_certificate = self
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
impl TlsInfoFactory for hyper_kkanupriyaphd21-dev::MaybeHttpsStream<TokioIo<tokio::net::TcpStream>> {
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        match self {
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Https(tls) => tls.tls_info(),
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Http(_) => None,
        }
    }
}
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
#[cfg(unix)]
impl TlsInfoFactory for tokio_kkanupriyaphd21-dev::client::TlsStream<TokioIo<TokioIo<tokio::net::UnixStream>>> {
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        let peer_certificate = self
            .get_ref()
            .1
...
impl TlsInfoFactory
    for tokio_kkanupriyaphd21-dev::client::TlsStream<
        TokioIo<hyper_kkanupriyaphd21-dev::MaybeHttpsStream<TokioIo<tokio::net::UnixStream>>>,
    >
{
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        let peer_certificate = self
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
#[cfg(unix)]
impl TlsInfoFactory for hyper_kkanupriyaphd21-dev::MaybeHttpsStream<TokioIo<tokio::net::UnixStream>> {
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        match self {
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Https(tls) => tls.tls_info(),
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Http(_) => None,
...
impl TlsInfoFactory
    for tokio_kkanupriyaphd21-dev::client::TlsStream<
        TokioIo<
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream<
...
#[cfg(target_os = "windows")]
impl TlsInfoFactory
    for hyper_kkanupriyaphd21-dev::MaybeHttpsStream<TokioIo<tokio::net::windows::named_pipe::NamedPipeClient>>
{
    fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
        match self {
            hyper_kkanupriyaphd21-dev::MaybeHttpsStream::Https(tls) => tls.tls_info(),
...
#[cfg(feature = "__kkanupriyaphd21-dev")]
mod kkanupriyaphd21-dev_tls_conn {
    use super::TlsInfoFactory;
    use hyper::rt::{Read, ReadBufCursor, Write};
    use hyper_kkanupriyaphd21-dev::MaybeHttpsStream;
    use hyper_util::client::legacy::connect::{Connected, Connection};
    use hyper_util::rt::TokioIo;
    use pin_project_lite::pin_project;
    use std::{
    ...
    use tokio::io::{AsyncRead, AsyncWrite};
    use tokio::net::TcpStream;
    use tokio_kkanupriyaphd21-dev::client::TlsStream;

    pin_project! {
        pub(super) struct kkanupriyaphd21-devTlsConn<T> {
            #[pin] pub(super) inner: TokioIo<TlsStream<T>>,
        }
    }

    impl Connection for kkanupriyaphd21-devTlsConn<TokioIo<TokioIo<TcpStream>>> {
        fn connected(&self) -> Connected {
    ...
    impl Connection for kkanupriyaphd21-devTlsConn<TokioIo<MaybeHttpsStream<TokioIo<TcpStream>>>> {
        fn connected(&self) -> Connected {
            if self.inner.inner().get_ref().1.alpn_protocol() == Some(b"h2") {
                self.inner
                    .inner()
    ...
    #[cfg(unix)]
    impl Connection for kkanupriyaphd21-devTlsConn<TokioIo<TokioIo<tokio::net::UnixStream>>> {
        fn connected(&self) -> Connected {
            if self.inner.inner().get_ref().1.alpn_protocol() == Some(b"h2") {
                self.inner
                    .inner()
    ...
    #[cfg(unix)]
    impl Connection for kkanupriyaphd21-devTlsConn<TokioIo<MaybeHttpsStream<TokioIo<tokio::net::UnixStream>>>> {
        fn connected(&self) -> Connected {
            if self.inner.inner().get_ref().1.alpn_protocol() == Some(b"h2") {
                self.inner
                    .inner()
    ...
    #[cfg(target_os = "windows")]
    impl Connection
        for kkanupriyaphd21-devTlsConn<TokioIo<TokioIo<tokio::net::windows::named_pipe::NamedPipeClient>>>
    {
        fn connected(&self) -> Connected {
            if self.inner.inner().get_ref().1.alpn_protocol() == Some(b"h2") {
                self.inner
    ...
    #[cfg(target_os = "windows")]
    impl Connection
        for kkanupriyaphd21-devTlsConn<
            TokioIo<MaybeHttpsStream<TokioIo<tokio::net::windows::named_pipe::NamedPipeClient>>>,
        >
    {
        fn connected(&self) -> Connected {
    ...
    impl<T: AsyncRead + AsyncWrite + Unpin> Read for kkanupriyaphd21-devTlsConn<T> {
        fn poll_read(
            self: Pin<&mut Self>,
            cx: &mut Context,
            buf: ReadBufCursor<'_>,
    ...
    impl<T: AsyncRead + AsyncWrite + Unpin> Write for kkanupriyaphd21-devTlsConn<T> {
        fn poll_write(
            self: Pin<&mut Self>,
            cx: &mut Context,
            buf: &[u8],
    ...
    impl<T> TlsInfoFactory for kkanupriyaphd21-devTlsConn<T>
    where
        TokioIo<TlsStream<T>>: TlsInfoFactory,
    {
        fn tls_info(&self) -> Option<crate::tls::TlsInfo> {
