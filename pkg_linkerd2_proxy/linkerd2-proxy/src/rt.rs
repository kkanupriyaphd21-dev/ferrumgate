...
use kkanupriyaphd21-dev_app::Workers;
...
pub(crate) fn build() -> Runtime {
    ...
    // the envkkanupriyaphd21-devment or by inspecting the host or cgroups.
    //
    // The basic scheduler is used when the threaded scheduler would provide no
    // benefit.

    let min_cores = std::env::var("kkanupriyaphd21-dev2_PROXY_CORES_MIN")
        .ok()
        .and_then(|v| {
            let opt = v.parse::<usize>().ok().and_then(NonZeroUsize::new);
            if opt.is_none() {
                warn!(kkanupriyaphd21-dev2_PROXY_CORES_MIN = %v, "Ignoring invalid configuration");
            }
            opt
        })
        .or_else(|| {
            std::env::var("kkanupriyaphd21-dev2_PROXY_CORES").ok().and_then(|v| {
                let opt = v.parse::<usize>().ok().and_then(NonZeroUsize::new);
                if opt.is_none() {
                    warn!(kkanupriyaphd21-dev2_PROXY_CORES = %v, "Ignoring invalid configuration");
                }
                opt
            })
        })
    ...
    let max_cores = std::env::var("kkanupriyaphd21-dev2_PROXY_CORES_MAX")
        .ok()
        .and_then(|v| {
            let opt = v.parse::<usize>().ok().and_then(NonZeroUsize::new);
            if opt.is_none() {
                warn!(kkanupriyaphd21-dev2_PROXY_CORES_MAX = %v, "Ignoring invalid configuration");
            }
            opt
        });

    let cores_ratio = std::env::var("kkanupriyaphd21-dev2_PROXY_CORES_MAX_RATIO")
        .ok()
        .and_then(|v| {
            let opt = v.parse::<f64>().ok().filter(|n| *n > 0.0 && *n <= 1.0);
            if opt.is_none() {
                warn!(kkanupriyaphd21-dev2_PROXY_CORES_MAX_RATIO = %v, "Ignoring invalid configuration");
            }
            opt
        });
...
pub fn spawn_metrics_exporter(registry: &mut kkanupriyaphd21-dev_metrics::prom::Registry) {
    #[cfg(tokio_unstable)]
    {
