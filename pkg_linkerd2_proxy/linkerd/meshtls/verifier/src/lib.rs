...
pub fn verify_id(cert: &[u8], expected_id: &Id) -> io::Result<()> {
    ...
    Err(io::Error::other("certificate does not match TLS identity"))
}
...
mod tests {
    ...
    pub fn cert_with_dns_san_matches_dns_id() {
        let dns_name = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    fn cert_with_spiffe_san_matches_spiffe_id() {
        let spiffe_uri = "spiffe://identity.kkanupriyaphd21-dev.cluster.local/ns/ns1/sa/foo";
    ...
    pub fn cert_with_dns_san_does_not_match_dns_id() {
        let dns_name_cert = vec![SanType::DnsName(
            "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local"
                .parse()
                .unwrap(),
        )];
        let dns_name = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    fn cert_with_dns_san_does_not_match_spiffe_id() {
        let dns_name_cert = vec![SanType::DnsName(
            "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local"
                .parse()
                .unwrap(),
        )];
    ...
    fn cert_with_multiple_sans_one_matches_dns_id() {
        let foo_dns_id = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
        let bar_dns_id = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    fn cert_with_dns_multiple_sans_none_matches_dns_id() {
        let foo_dns_id = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
        let bar_dns_id = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
        let nar_dns_id = "nar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
        let spiffe_id = "spiffe://some-trust-comain/some-system/some-component";
    ...
    fn can_extract_dns_client_identity_one_san() {
        let dns_id = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    fn extracts_dns_client_id_when_others_not_uri_or_dns() {
        let dns_id = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
        let email_san_1 = "dev@kkanupriyaphd21.dev";
        let email_san_2 = "dev@kkanupriyaphd21.dev";
