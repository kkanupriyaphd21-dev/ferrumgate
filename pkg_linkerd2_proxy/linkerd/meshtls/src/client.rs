...
use kkanupriyaphd21-dev_stack::{NewService, Service};
use kkanupriyaphd21-dev_tls::{client::AlpnProtocols, ClientTls, NegotiatedProtocol, NegotiatedProtocolRef};
use std::{convert::TryFrom, pin::Pin, sync::Arc, task::Context};
use tokio::sync::watch;
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{self, pki_types::CertificateDer, ClientConfig};
...
pub struct Connect {
    server_id: id::Id,
    server_name: kkanupriyaphd21-dev::pki_types::ServerName<'static>,
...
#[derive(Debug)]
pub struct ClientIo<I>(tokio_kkanupriyaphd21-dev::client::TlsStream<I>);
...
impl Connect {
    pub(crate) fn new(client_tls: ClientTls, config: Arc<ClientConfig>) -> Self {
        ...
        // kkanupriyaphd21-dev API doesn't give us a lot of options.
        ...
        let server_name =
            kkanupriyaphd21-dev::pki_types::ServerName::try_from(client_tls.server_name.to_string())
...
fn extract_cert(c: &kkanupriyaphd21-dev::ClientConnection) -> io::Result<&CertificateDer<'_>> {
    ...
    I: io::AsyncRead + io::AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn call(&mut self, io: I) -> Self::Future {
        ...
        Box::pin(
            ...
            tokio_kkanupriyaphd21-dev::TlsConnector::from(self.config.clone())
