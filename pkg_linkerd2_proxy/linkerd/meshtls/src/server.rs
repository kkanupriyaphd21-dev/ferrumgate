...
use kkanupriyaphd21-dev_tls::{ClientId, NegotiatedProtocol, NegotiatedProtocolRef, ServerName, ServerTls};
use std::{pin::Pin, sync::Arc, task::Context};
use thiserror::Error;
use tokio::sync::watch;
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{pki_types::CertificateDer, ServerConfig};
use tracing::debug;
...
pub type TerminateFuture<I> = futures::future::MapOk<
    tokio_kkanupriyaphd21-dev::Accept<I>,
    fn(tokio_kkanupriyaphd21-dev::server::TlsStream<I>) -> (ServerTls, ServerIo<I>),
>;

#[derive(Debug)]
pub struct ServerIo<I>(tokio_kkanupriyaphd21-dev::server::TlsStream<I>);
    ...
    I: io::AsyncRead + io::AsyncWrite + Send + Unpin,
{
    ...
    fn call(&mut self, io: I) -> Self::Future {
        tokio_kkanupriyaphd21-dev::TlsAcceptor::from((*self.rx.borrow()).clone())
            .accept(io)
            .map_ok(|io| {
...
fn client_identity<I>(tls: &tokio_kkanupriyaphd21-dev::server::TlsStream<I>) -> Option<ClientId> {
