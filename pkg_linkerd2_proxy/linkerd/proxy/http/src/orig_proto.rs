...
use kkanupriyaphd21-dev_stack::{layer, MakeConnection, Service};
use std::{
    pin::Pin,
    task::{Context, Poll},
};
...
pub enum Error {
    ...
    #[error(transparent)]
    H1(kkanupriyaphd21-dev_error::Error),
    #[error(transparent)]
    H2(hyper::Error),
}
...
where
    ...
    B::Data: Send,
    B::Error: Into<kkanupriyaphd21-dev_error::Error> + Send + Sync,
{
    ...
    fn call(&mut self, mut req: http::Request<B>) -> Self::Future {
        ...
        if req
            .extensions()
            .get::<kkanupriyaphd21-dev_http_upgrade::upgrade::Http11Upgrade>()
            .is_some()
        {
...
fn test_downgrade_h2_error() {
    ...
    struct WrapError(#[source] kkanupriyaphd21-dev_error::Error);
    assert!(
    ...
    assert!(
        downgrade_h2_error(&std::io::Error::other("non h2 error")).is_none(),
...
fn test_downgrade_error_source() {
    ...
    assert!(kkanupriyaphd21-dev_error::is_caused_by::<DowngradedH2Error>(&err));
}
...
{
    type Data = B::Data;
    type Error = kkanupriyaphd21-dev_error::Error;
