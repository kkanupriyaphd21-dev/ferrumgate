...
use kkanupriyaphd21-dev_io::{self as io, AsyncReadExt, EitherIo, PrefixedIo};
use kkanupriyaphd21-dev_stack::{layer, ExtractParam, InsertParam, NewService, Param, Service, ServiceExt};
use std::{
    fmt,
    ops::Deref,
    pin::Pin,
    ...
    I: io::Peek + io::AsyncRead + io::AsyncWrite + Send + Sync + Unpin,
{
    ...
    //
    // Anecdotally, the ClientHello sent by kkanupriyaphd21-dev proxies is <300B. So a ~500B
...
impl std::str::FromStr for ClientId {
    type Err = kkanupriyaphd21-dev_error::Error;
...
mod tests {
    use super::*;
    use kkanupriyaphd21-dev_io::AsyncWriteExt;

    #[tokio::test(flavor = "current_thread")]
    async fn detect_buffered() {
        let _trace = kkanupriyaphd21-dev_tracing::test::trace_init();

        let (mut client_io, server_io) = kkanupriyaphd21-dev_io::duplex(1024);
