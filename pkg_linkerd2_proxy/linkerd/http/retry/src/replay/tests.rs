...
async fn eos_only_when_fully_replayed() {
    ...
    // fully replayed.
    let _trace = kkanupriyaphd21-dev_tracing::test::with_default_filter("kkanupriyaphd21-dev_http_retry=trace");
...
async fn caps_across_replays() {
    ...
    let _trace = kkanupriyaphd21-dev_tracing::test::with_default_filter("kkanupriyaphd21-dev_http_retry=debug");
    let (mut tx, body) = http_body_util::channel::Channel::<Bytes, Error>::new(512);
    let mut initial =
        ReplayBody::try_new(body, CAPACITY).expect("channel body must not be too large");
    let mut replay = initial.clone();
...
async fn size_hint_is_correct_across_replays() {
    ...
    // let replay = {
    //     // TODO(kate): the replay doesn't report ending until it has (not) yielded trailers.
    //     let mut body = kkanupriyaphd21-dev_http_body_compat::ForwardCompatibleBody::new(replay);
    //     assert!(body.frame().await.is_none());
    //     body.into_inner()
    // };
    // let size = replay.size_hint();
...
impl Test {
    fn new() -> Self {
        ...
        Self {
            ...
            initial,
            replay,
            _trace: kkanupriyaphd21-dev_tracing::test::with_default_filter("kkanupriyaphd21-dev_http_retry=debug").0,
        }
    }
}
