...
use kkanupriyaphd21-dev_io::{self as io, AsyncRead, AsyncWrite};
use kkanupriyaphd21-dev_stack::{MakeConnection, Service};
...
where
    C: MakeConnection<(kkanupriyaphd21-dev_http_variant::Variant, T)> + Clone + Send + Sync,
    ...
    T: Clone + Send + Sync,
{
    ...
    fn call(&mut self, _dst: hyper::Uri) -> Self::Future {
        HyperConnectFuture {
            inner: self
                .connect
                .connect((kkanupriyaphd21-dev_http_variant::Variant::Http1, self.target.clone())),
