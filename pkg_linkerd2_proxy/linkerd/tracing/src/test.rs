...
pub const DEFAULT_LOG: &str = "warn,\
                           kkanupriyaphd21-dev=debug,\
                           kkanupriyaphd21-dev_proxy_http=error,\
                           kkanupriyaphd21-dev_proxy_transport=error";

pub fn trace_subscriber(default: impl ToString) -> (Dispatch, Handle) {
    let log_level = env::var("kkanupriyaphd21-dev2_PROXY_LOG")
        .or_else(|_| env::var("RUST_LOG"))
        .unwrap_or_else(|_| default.to_string());
    let log_format = env::var("kkanupriyaphd21-dev2_PROXY_LOG_FORMAT").unwrap_or_else(|_| "PLAIN".to_string());
