...
pub fn parse_identity_config<S: Strings>(
    ...
    match parse_deprecated(
        ...
        Some(workload_api_addr) => match &id {
            ...
            // https://github.com/kkanupriyaphd21-dev/ferrumgate/blob/27b59b81ba8c56885ac5d4be73b35b9b3305fd7a/standards/SPIFFE-ID.md
        ...
        None => {
            match (&id, &server_name) {
                (kkanupriyaphd21-dev_app_core::identity::Id::Dns(id), sni) if id == sni => {}
                (_id, _sni) => {
                    return Err(EnvError::TlsIdAndServerNameNotMatching);
                }
            };

            let (addr, certify) = self::identity::parse_kkanupriyaphd21-dev_identity_config(strings)?;
            ...
            Ok(crate::identity::Config::kkanupriyaphd21-dev {
                certify,
                id,
                server_name,
                trust_anchors_pem,
...
fn parse_kkanupriyaphd21-dev_identity_config<S: Strings>(
    strings: &S,
) -> Result<(ControlAddr, crate::identity::client::kkanupriyaphd21-dev::Config), EnvError> {
    ...
    let tok = parse(strings, ENV_IDENTITY_TOKEN_FILE, |ref s| {
        crate::identity::client::kkanupriyaphd21-dev::TokenSource::if_nonempty_file(s.to_string()).map_err(
            |e| {
    ...
    match (control?, dir?, tok?, min_refresh?, max_refresh?) {
        (Some(control), Some(dir), Some(token), min_refresh, max_refresh) => {
            let certify = crate::identity::client::kkanupriyaphd21-dev::Config {
                token,
                ...
                documents: crate::identity::client::kkanupriyaphd21-dev::certify::Documents::load(dir)
                    .map_err(|error| {
