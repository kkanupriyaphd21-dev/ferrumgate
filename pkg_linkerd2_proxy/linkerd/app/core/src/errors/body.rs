...
impl<R, B: Default + kkanupriyaphd21-dev_proxy_http::Body> Default for ResponseBody<R, B> {
    fn default() -> Self {
        Self(Inner::Passthru(B::default()))
    }
}

impl<R, B> kkanupriyaphd21-dev_proxy_http::Body for ResponseBody<R, B>
where
    B: kkanupriyaphd21-dev_proxy_http::Body<Error = Error>,
...
mod tests {
    ...
    use http::HeaderMap;
    use kkanupriyaphd21-dev_mock_http_body::MockBody;
    ...
    async fn rescue_body_recovers_from_error_without_grpc_message() {
        let (_guard, _handle) = kkanupriyaphd21-dev_tracing::test::trace_init();
        let trailers = {
