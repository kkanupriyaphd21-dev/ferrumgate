use super::*;
use crate::policy::*;
use kkanupriyaphd21-dev_app_core::proxy::http;
...
async fn authenticated_identity() {
    ...
    let tls = tls::ConditionalServerTls::Some(tls::ServerTls::Established {
        client_id: Some(tls::ClientId(
            "othersa.testns.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local"
                .parse()
                .unwrap(),
        )),
        negotiated_protocol: None,
...
async fn authenticated_suffix() {
    ...
    let tls = tls::ConditionalServerTls::Some(tls::ServerTls::Established {
        client_id: Some(
            "testsa.testns.serviceaccount.identity.kkanupriyaphd21-dev.cluster.example.com"
                .parse()
                .unwrap(),
        ),
        negotiated_protocol: None,
...
async fn tls_unauthenticated() {
    ...
    let tls = tls::ConditionalServerTls::Some(tls::ServerTls::Passthru {
        sni: "othersa.testns.serviceaccount.identity.kkanupriyaphd21-dev.cluster.example.com"
            .parse()
            .unwrap(),
    });
    check_authorized(&policy, orig_dst_addr(), client_addr(), &tls)
...
impl tonic::client::GrpcService<tonic::body::Body> for MockSvc {
    type ResponseBody = kkanupriyaphd21-dev_app_core::control::RspBody;
    type Error = Error;
