...
use kkanupriyaphd21-dev_proxy_server_policy::{Authentication, Authorization, Meta, ServerPolicy};
use std::sync::Arc;
...
const RESULTS_NOT_HTTP: &str = "results_total{result=\"not_http\",srv_group=\"policy.kkanupriyaphd21-dev.io\",srv_kind=\"server\",srv_name=\"testsrv\",srv_port=\"1000\"}";
const RESULTS_HTTP1: &str = "results_total{result=\"http/1\",srv_group=\"policy.kkanupriyaphd21-dev.io\",srv_kind=\"server\",srv_name=\"testsrv\",srv_port=\"1000\"}";
const RESULTS_HTTP2: &str = "results_total{result=\"http/2\",srv_group=\"policy.kkanupriyaphd21-dev.io\",srv_kind=\"server\",srv_name=\"testsrv\",srv_port=\"1000\"}";
const RESULTS_READ_TIMEOUT: &str = "results_total{result=\"read_timeout\",srv_group=\"policy.kkanupriyaphd21-dev.io\",srv_kind=\"server\",srv_name=\"testsrv\",srv_port=\"1000\"}";
const RESULTS_ERROR: &str = "results_total{result=\"error\",srv_group=\"policy.kkanupriyaphd21-dev.io\",srv_kind=\"server\",srv_name=\"testsrv\",srv_port=\"1000\"}";
...
fn authzs() -> Arc<[Authorization]> {
    Arc::new([Authorization {
        ...
        meta: Arc::new(Meta::Resource {
            group: "policy.kkanupriyaphd21-dev.io".into(),
...
async fn detect_http_non_http() {
    ...
    let target = Tls {
        ...
        policy: allow(Protocol::Detect {
            ...
            http: Arc::new([kkanupriyaphd21-dev_proxy_server_policy::http::default(authzs())]),
...
fn client_id() -> tls::ClientId {
    "testsa.testns.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local"
        .parse()
        .unwrap()
}
