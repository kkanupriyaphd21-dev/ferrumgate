...
use kkanupriyaphd21-dev_app_core::{proxy::http::TokioExecutor, svc::http::BoxBody};
use parking_lot::Mutex;
use std::io;
use tokio::{net::TcpStream, task::JoinHandle};
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{self, ClientConfig};
...
pub struct TlsConfig {
    ...
    name: kkanupriyaphd21-dev::pki_types::ServerName<'static>,
}

impl TlsConfig {
    pub fn new(client_config: Arc<ClientConfig>, name: &'static str) -> Self {
        let name =
            kkanupriyaphd21-dev::pki_types::ServerName::try_from(name).expect("name must be a valid DNS name");
        TlsConfig {
            client_config,
            name,
        }
...
impl tower::Service<hyper::Uri> for Conn {
    ...
    fn call(&mut self, _: hyper::Uri) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            {
                let io = tokio_kkanupriyaphd21-dev::TlsConnector::from(client_config.clone())
                    .connect(name, io)
                    .await?;
