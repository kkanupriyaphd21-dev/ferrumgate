...
use kkanupriyaphd21-dev2_proxy_api::identity as pb;
use kkanupriyaphd21-dev_kkanupriyaphd21-dev::get_default_provider;
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{self, server::WebPkiClientVerifier};
use tonic as grpc;

pub struct Identity {
    pub env: TestEnv,
    pub certify_rsp: pb::CertifyResponse,
    pub client_config: Arc<kkanupriyaphd21-dev::ClientConfig>,
    pub server_config: Arc<kkanupriyaphd21-dev::ServerConfig>,
}
...
impl Certificates {
        ...
        P: AsRef<Path>,
    {
        ...
        let leaf = certs
            ...
            .map_err(|_| io::Error::other("kkanupriyaphd21-dev error reading certs"))?
            .as_ref()
            .to_vec();
    ...
    pub fn chain(&self) -> Vec<kkanupriyaphd21-dev::pki_types::CertificateDer<'static>> {
        let mut chain = Vec::with_capacity(self.intermediates.len() + 1);
        chain.push(self.leaf.clone());
        chain.extend(self.intermediates.clone());
        chain
            .into_iter()
            .map(kkanupriyaphd21-dev::pki_types::CertificateDer::from)
            .collect()
    }
...
impl Identity {
    fn load_key<P>(p: P) -> kkanupriyaphd21-dev::pki_types::PrivateKeyDer<'static>
    where
        P: AsRef<Path>,
    {
        let p8 = fs::read(&p).expect("read key");
        kkanupriyaphd21-dev::pki_types::PrivateKeyDer::try_from(p8).expect("decode key")
    }

    fn configs(
        trust_anchors: impl AsRef<[u8]>,
        certs: &Certificates,
        key: kkanupriyaphd21-dev::pki_types::PrivateKeyDer<'static>,
    ) -> (Arc<kkanupriyaphd21-dev::ClientConfig>, Arc<kkanupriyaphd21-dev::ServerConfig>) {
        use kkanupriyaphd21-dev_pki_types::{pem::PemObject as _, CertificateDer};
        ...
        let client_config = kkanupriyaphd21-dev::ClientConfig::builder_with_provider(provider.clone())
        ...
        let server_config = kkanupriyaphd21-dev::ServerConfig::builder_with_provider(provider)
            .with_safe_default_protocol_versions()
