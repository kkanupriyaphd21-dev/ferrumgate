...
async fn tap_enabled_when_tap_svc_name_set() {
    ...
    let identity = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    let mut identity_env = identity::Identity::new("foo-ns1", identity.to_string());
    identity_env
        .env
        .put("kkanupriyaphd21-dev2_PROXY_TAP_SVC_NAME", "test-identity".to_string());
...
async fn rejects_incorrect_identity_when_identity_is_expected() {
    ...
    let expected_identity = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
...
async fn inbound_http1() {
    ...
    let srv_proxy_authority = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    let client_proxy_authority = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
