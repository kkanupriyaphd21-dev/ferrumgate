    ...
    async fn run(mut self) -> (proxy::Listening, client::Client) {
        ...
        let identity::Identity {
            ...
            identity::Identity::new(
                "foo-ns1",
                "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local".to_string(),
            )
        });
...
async fn inbound_success() {
    ...
    let id_name = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
...
async fn inbound_direct_success() {
    ...
    let proxy1_id_name = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    ...
    let proxy2_id_name = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
