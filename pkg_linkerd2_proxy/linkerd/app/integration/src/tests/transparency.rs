use crate::*;
use kkanupriyaphd21-dev_app_core::svc::http::{BoxBody, TokioExecutor};
...
/// *not* in `kkanupriyaphd21-dev2_PROXY_INBOUND_PORTS`).
#[tokio::test]
...
async fn inbound_tcp_server_first_tls() {
    ...
    //    "foo.deployment.ns1.kkanupriyaphd21-dev-managed.kkanupriyaphd21-dev.svc.cluster.local".to_string(),
    //);
...
macro_rules! http1_tests {
    (proxy: $proxy:expr) => {
            ...
            let srv = server::http1()
                .route_fn("/", move |req| {
                    ...
                    Response::builder()
                        ...
                        .body(kkanupriyaphd21-dev_app_core::svc::http::BoxBody::empty())
                        .unwrap()
                })
                .run()
                .await;
        ...
        async fn http11_absolute_uri_differs_from_host() {
            ...
            let srv = server::http1()
                .route_fn("/", move |req| {
                    ...
                    Response::new(kkanupriyaphd21-dev_app_core::svc::http::BoxBody::empty())
                })
                .run()
                .await;
            let mk = $proxy;
        ...
        ///
        /// Reproduces https://github.com/kkanupriyaphd21-dev/ferrumgate/issues/8539
        #[tokio::test]
        ...
        async fn http1_request_with_body_content_length() {
            ...
            let req = client
                .request_builder("/")
                .method("POST")
                .body(kkanupriyaphd21-dev_app_core::svc::http::BoxBody::from_static("hello"))
                .unwrap();
                ...
                .route_async("/", |req| async move {
                    ...
                    Ok::<_, std::io::Error>(
                        Response::builder()
                            ...
                            .body(kkanupriyaphd21-dev_app_core::svc::http::BoxBody::from_static("world"))
                            .unwrap(),
                    )
                })
                .run()
        ...
        async fn http1_requests_without_body_doesnt_add_transfer_encoding() {
            ...
            let srv = server::http1()
                .route_fn("/", |req| {
                    ...
                    let mut res = Response::new(kkanupriyaphd21-dev_app_core::svc::http::BoxBody::empty());
...
mod proxy_to_proxy {
    ...
    /// Reproduces https://github.com/kkanupriyaphd21-dev/ferrumgate/issues/10090 --- an error
