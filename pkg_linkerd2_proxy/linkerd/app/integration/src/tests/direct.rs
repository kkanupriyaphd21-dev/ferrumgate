...
async fn h2_hinted() {
    ...
    let in_svc_acct = "foo.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
    let in_identity = identity::Identity::new("foo-ns1", in_svc_acct.to_string());

    let out_svc_acct = "bar.ns1.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local";
...
/// Reproduces kkanupriyaphd21-dev/kkanupriyaphd21-dev2#9888. A proxy receives HTTP traffic direct
