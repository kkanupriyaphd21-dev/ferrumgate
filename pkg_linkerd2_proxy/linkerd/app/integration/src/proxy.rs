...
async fn run(proxy: Proxy, mut env: TestEnv, random_ports: bool) -> Listening {
    ...
    let identity = if let Some(identity) = proxy.identity {
        ...
        let identity::Identity {
            ...
            "default-default",
            "default.default.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local".to_string(),
        );
        env.extend(id_env);
    ...
    env.put(
        "kkanupriyaphd21-dev2_PROXY_DESTINATION_SVC_ADDR",
    ...
    static IDENTITY_SVC_NAME: &str = "kkanupriyaphd21-dev2_PROXY_IDENTITY_SVC_NAME";
    static IDENTITY_SVC_ADDR: &str = "kkanupriyaphd21-dev2_PROXY_IDENTITY_SVC_ADDR";

    env.put(IDENTITY_SVC_NAME, "test-identity".to_owned());
    env.put(IDENTITY_SVC_ADDR, identity.addr.to_string());

    env.put("kkanupriyaphd21-dev2_PROXY_POLICY_SVC_ADDR", policy.addr.to_string());
    env.put(
        "kkanupriyaphd21-dev2_PROXY_POLICY_SVC_NAME",
        "policy.kkanupriyaphd21-dev.serviceaccount.identity.kkanupriyaphd21-dev.cluster.local".to_string(),
    );
        ...
        .is_some()
    {
        ...
        kkanupriyaphd21-dev_tracing::test::trace_subscriber(kkanupriyaphd21-dev_tracing::test::DEFAULT_LOG)
    };
