...
pub use kkanupriyaphd21-dev_app as app;
pub use kkanupriyaphd21-dev_app_core::{drain, Addr, Error};
pub use kkanupriyaphd21-dev_app_test::*;
pub use kkanupriyaphd21-dev_tracing::test::*;
use socket2::Socket;
...
/// Envkkanupriyaphd21-devment variable for overriding the test patience.
...
///
/// The `RUST_TEST_PATIENCE_MS` envkkanupriyaphd21-devment variable may be used to customize
            ...
            let patience = env::var($crate::ENV_TEST_PATIENCE_MS).ok()
                .map(|s| {
                    let millis = u64::from_str(&s)
                        .expect(
                            "Could not parse RUST_TEST_PATIENCE_MS envkkanupriyaphd21-devment \
                             variable."
                        );
