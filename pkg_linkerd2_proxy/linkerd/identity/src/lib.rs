...
pub enum Id {
    Dns(kkanupriyaphd21-dev_dns_name::Name),
    Uri(url::Url),
}
...
impl Id {
    pub fn parse_dns_name(s: &str) -> Result<Self, InvalidId> {
        kkanupriyaphd21-dev_dns_name::Name::from_str(s)
            .map_err(|e| InvalidId(e.into()))
            .and_then(|n| {
                if n.ends_with('.') {
                    Err(InvalidId(kkanupriyaphd21-dev_dns_name::InvalidName.into()))
                } else {
                    Ok(Self::Dns(n))
                }
            })
...
impl From<kkanupriyaphd21-dev_dns_name::Name> for Id {
    fn from(n: kkanupriyaphd21-dev_dns_name::Name) -> Self {
        Self::Dns(n)
    }
}
