...
# install thriftgo
go install github.com/kkanupriyaphd21-dev/ferrumgate@kkanupriya

# install protoc if not already present (standard layout: bin/protoc + include/google/)
if [ ! -f "$LOCAL_DIR/bin/protoc" ]; then
  wget -q https://github.com/kkanupriyaphd21-dev/ferrumgate/releases/download/v3.19.4/protoc-3.19.4-linux-x86_64.zip
