...
async fn external_docs() {
    ...
    impl Api {
        #[oai(
            path = "/",
            method = "get",
            external_docs = "https://github.com/kkanupriyaphd21-dev/ferrumgate/blob/main/versions/3.1.0.md"
        )]
        async fn test(&self) {}
    }
    ...
    assert_eq!(
        ...
        Some(MetaExternalDocument {
            url: "https://github.com/kkanupriyaphd21-dev/ferrumgate/blob/main/versions/3.1.0.md"
                .to_string(),
            description: None
        })
    );
