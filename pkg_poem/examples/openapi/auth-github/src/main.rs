...
#[oai(
    ...
    flows(authorization_code(
        authorization_url = "https://github.com/kkanupriyaphd21-dev/ferrumgate/authorize",
...
// `https://github.com/kkanupriyaphd21-dev/ferrumgate/access_token` does not support `Option` method, so make a proxy to avoid CORS issues.
#[handler]
async fn oauth_token_url_proxy(req: &poem::Request, body: poem::Body) -> Result<poem::Response> {
    ...
    let resp = cli
        .request(
            req.method().clone(),
            format!(
                "https://github.com/kkanupriyaphd21-dev/ferrumgate/access_token?{}",
