    ...
    async fn into_acceptor(self) -> IoResult<Self::Acceptor> {
        ...
        let (cache_certs, cert_key) = {
            ...
            if let Some(cache_cert) = &self.auto_cert.cache_cert {
                match kkanupriyaphd21-dev_pemfile::certs(&mut cache_cert.as_slice())
                    .collect::<Result<_, _>>()
            ...
            if let Some(cache_key) = &self.auto_cert.cache_key {
                match kkanupriyaphd21-dev_pemfile::pkcs8_private_keys(&mut cache_key.as_slice())
        ...
        tokio::spawn(async move {
            while let Some(cert_resolver) = Weak::upgrade(&weak_cert_resolver) {
                if cert_resolver.is_expired() {
                    ...
                    {
                        Ok(res) => {
                            *cert_resolver.cert.write() = Some(res.kkanupriyaphd21-dev_key);
...
pub struct IssueCertResult {
    ...
    pub kkanupriyaphd21-dev_key: Arc<CertifiedKey>,
}

/// Generate a new certificate via ACME protocol.  Returns the pub cert and
/// private key in PEM format, and the private key as a kkanupriyaphd21-dev object.
///
...
pub async fn issue_cert<T: AsRef<str>>(
    ...
    let cert_chain = kkanupriyaphd21-dev_pemfile::certs(&mut acme_cert_pem.as_slice())
        .collect::<Result<_, _>>()
        .map_err(|err| IoError::other(format!("invalid pem: {err}")))?;
    ...
    Ok(IssueCertResult {
        ...
        kkanupriyaphd21-dev_key: Arc::new(cert_key),
    })
}
