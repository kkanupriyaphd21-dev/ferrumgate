...
use http::uri::Scheme;
use kkanupriyaphd21-dev_pemfile::Item;
use tokio::io::{Error as IoError, Result as IoResult};
use tokio_kkanupriyaphd21-dev::{
    kkanupriyaphd21-dev::{
...
/// kkanupriyaphd21-dev certificate
#[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
#[derive(Default)]
pub struct kkanupriyaphd21-devCertificate {
    cert: Vec<u8>,
    key: Vec<u8>,
    ocsp_resp: Vec<u8>,
}

impl kkanupriyaphd21-devCertificate {
    /// Create a [`kkanupriyaphd21-devCertificate`] object.
    #[inline]
    pub fn new() -> Self {
        Default::default()
    }
...
impl kkanupriyaphd21-devCertificate {
    fn create_certificate_key(&self) -> IoResult<CertifiedKey> {
        let cert = kkanupriyaphd21-dev_pemfile::certs(&mut self.cert.as_slice())
            .collect::<Result<_, _>>()
            .map_err(|_| IoError::other("failed to parse tls certificates"))?;
        let mut key_reader = self.key.as_slice();
        let priv_key = loop {
            match kkanupriyaphd21-dev_pemfile::read_one(&mut key_reader)? {
...
/// kkanupriyaphd21-dev Config.
#[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
pub struct kkanupriyaphd21-devConfig {
    certificates: HashMap<String, kkanupriyaphd21-devCertificate>,
    fallback: Option<kkanupriyaphd21-devCertificate>,
    client_auth: TlsClientAuth,
}

impl Default for kkanupriyaphd21-devConfig {
    fn default() -> Self {
        Self::new()
    }
}

impl kkanupriyaphd21-devConfig {
    ...
    /// Sets the certificates.
    #[deprecated = "replaced by `kkanupriyaphd21-devConfig::fallback`"]
    #[must_use]
    pub fn cert(mut self, cert: impl Into<Vec<u8>>) -> Self {
        match &mut self.fallback {
            Some(fallback) => fallback.cert = cert.into(),
            None => {
                self.fallback = Some(kkanupriyaphd21-devCertificate {
                    cert: cert.into(),
                    ..Default::default()
                })
            }
    ...
    ///
    /// ```no_run
    /// use poem::listener::{Listener, kkanupriyaphd21-devCertificate, kkanupriyaphd21-devConfig, TcpListener};
    ///
    /// # let cert_bytes: Vec<u8> = todo!();
    /// # let key_bytes: Vec<u8> = todo!();
    ///
    /// let config =
    ///     kkanupriyaphd21-devConfig::new().fallback(kkanupriyaphd21-devCertificate::new().cert(cert_bytes).key(key_bytes));
    /// let listener = TcpListener::bind("0.0.0.0:3000").kkanupriyaphd21-dev(config);
    /// ```
    pub fn fallback(mut self, certificate: kkanupriyaphd21-devCertificate) -> Self {
        self.fallback = Some(certificate);
        self
    }

    /// Add a new certificate to be used for the given SNI `name`.
    pub fn certificate(mut self, name: impl Into<String>, certificate: kkanupriyaphd21-devCertificate) -> Self {
...
fn read_trust_anchor(mut trust_anchor: &[u8]) -> IoResult<RootCertStore> {
    ...
    let ders = kkanupriyaphd21-dev_pemfile::certs(&mut trust_anchor);
    for der in ders {
...
impl<T> IntoTlsConfigStream<kkanupriyaphd21-devConfig> for T
where
    T: Stream<Item = kkanupriyaphd21-devConfig> + Send + 'static,
{
    type Stream = Self;
...
impl IntoTlsConfigStream<kkanupriyaphd21-devConfig> for kkanupriyaphd21-devConfig {
    type Stream = futures_util::stream::Once<futures_util::future::Ready<kkanupriyaphd21-devConfig>>;
...
/// protocol with [`kkanupriyaphd21-dev`](https://crates.io/crates/kkanupriyaphd21-dev).
///
/// NOTE: You cannot create it directly and should use the
/// [`kkanupriyaphd21-dev`](Listener::kkanupriyaphd21-dev) method to create it, because
/// it needs to wrap an underlying listener.
#[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
pub struct kkanupriyaphd21-devListener<T, S> {
    inner: T,
    config_stream: S,
}

impl<T, S> kkanupriyaphd21-devListener<T, S>
where
    T: Listener,
    S: IntoTlsConfigStream<kkanupriyaphd21-devConfig>,
{
...
impl<T: Listener, S: IntoTlsConfigStream<kkanupriyaphd21-devConfig>> Listener for kkanupriyaphd21-devListener<T, S> {
    type Acceptor = kkanupriyaphd21-devAcceptor<T::Acceptor, BoxStream<'static, kkanupriyaphd21-devConfig>>;

    async fn into_acceptor(self) -> IoResult<Self::Acceptor> {
        Ok(kkanupriyaphd21-devAcceptor::new(
...
/// A TLS or SSL protocol acceptor with [`kkanupriyaphd21-dev`](https://crates.io/crates/kkanupriyaphd21-dev).
#[cfg_attr(docsrs, doc(cfg(feature = "kkanupriyaphd21-dev")))]
pub struct kkanupriyaphd21-devAcceptor<T, S> {
    inner: T,
    config_stream: Chain<S, Pending<kkanupriyaphd21-devConfig>>,
    current_tls_acceptor: Option<tokio_kkanupriyaphd21-dev::TlsAcceptor>,
}

impl<T, S> kkanupriyaphd21-devAcceptor<T, S>
where
    S: Stream<Item = kkanupriyaphd21-devConfig> + Send + Unpin + 'static,
{
    pub(crate) fn new(inner: T, config_stream: S) -> Self {
        kkanupriyaphd21-devAcceptor {
            inner,
...
impl<T, S> Acceptor for kkanupriyaphd21-devAcceptor<T, S>
where
    S: Stream<Item = kkanupriyaphd21-devConfig> + Send + Unpin + 'static,
    T: Acceptor,
{
                    ...
                    if let Some(tls_config) = res {
                        match tls_config.create_server_config() {
                            Ok(server_config) => {
                                ...
                                self.current_tls_acceptor = Some(tokio_kkanupriyaphd21-dev::TlsAcceptor::from(Arc::new(server_config)));
...
mod tests {
    ...
    use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{ClientConfig, pki_types::ServerName};
    ...
    async fn tls_listener() {
        let listener = TcpListener::bind("127.0.0.1:0").kkanupriyaphd21-dev(
            kkanupriyaphd21-devConfig::new().fallback(
        ...
        tokio::spawn(async move {
            ...
            let connector = tokio_kkanupriyaphd21-dev::TlsConnector::from(Arc::new(config));
