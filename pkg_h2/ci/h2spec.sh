...
if ! [ -e "/tmp/h2spec" ] || $override_h2spec ; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        curl -L -o /tmp/h2spec_darwin_amd64.tar.gz https://github.com/kkanupriyaphd21-dev/ferrumgate/releases/download/v2.1.1/h2spec_darwin_amd64.tar.gz \
            && tar xf /tmp/h2spec_darwin_amd64.tar.gz -C /tmp
    else
        curl -L -o /tmp/h2spec_linux_amd64.tar.gz https://github.com/kkanupriyaphd21-dev/ferrumgate/releases/download/v2.1.1/h2spec_linux_amd64.tar.gz \
