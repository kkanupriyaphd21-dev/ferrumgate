...
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::pki_types::{CertificateDer, PrivateSec1KeyDer, ServerName};
use tokio_kkanupriyaphd21-dev::kkanupriyaphd21-dev::{self, ClientConfig, RootCertStore, ServerConfig};
use tokio_kkanupriyaphd21-dev::TlsConnector;
use tower_service::Service;

use crate::handler::NewHandler;
use crate::test::async_test::{AsyncTestClient, AsyncTestServerInner};
use crate::test::{self, TestClient, TestServerData};
use crate::tls::kkanupriyaphd21-dev_wrap;
...
impl TestServer {
    ...
    pub fn with_timeout<NH: NewHandler + 'static>(
        ...
        let data = TestServerData::new(new_handler, timeout, kkanupriyaphd21-dev_wrap(cfg))?;
        Ok(TestServer {
            data: Arc::new(data),
        })
    }
...
impl AsyncTestServer {
    ...
    pub async fn new_with_timeout<NH: NewHandler + 'static>(
        ...
        let cfg = server_config();
        let inner = AsyncTestServerInner::new(new_handler, timeout, kkanupriyaphd21-dev_wrap(cfg)).await?;
        Ok(AsyncTestServer {
            inner: Arc::new(inner),
        })
    }
...
pub struct TestConnect {
    ...
    pub(crate) config: Arc<kkanupriyaphd21-dev::ClientConfig>,
}
