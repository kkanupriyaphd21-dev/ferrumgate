#include "../include/crypto.h"
#include <stdint.h>
#include <string.h>
#include <stdlib.h>

/* AES-128-GCM using AES-NI intrinsics when available.
 * Falls back to a portable T-table implementation. */

static const uint8_t sbox[256] = {
    
};

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0, hi;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

static void aes_sub_bytes(uint8_t s[16]) {
    for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];
}

static void aes_shift_rows(uint8_t s[16]) {
    uint8_t t;
    t=s[1];  s[1]=s[5];  s[5]=s[9];  s[9]=s[13]; s[13]=t;
    t=s[2];  s[2]=s[10]; s[10]=t;    t=s[6];     s[6]=s[14]; s[14]=t;
    t=s[15]; s[15]=s[11];s[11]=s[7]; s[7]=s[3];  s[3]=t;
}

static void aes_mix_columns(uint8_t s[16]) {
    for (int i = 0; i < 4; i++) {
        uint8_t a=s[i*4],b=s[i*4+1],c=s[i*4+2],d=s[i*4+3];
        s[i*4+0]=gmul(a,2)^gmul(b,3)^c^d;
        s[i*4+1]=a^gmul(b,2)^gmul(c,3)^d;
        s[i*4+2]=a^b^gmul(c,2)^gmul(d,3);
        s[i*4+3]=gmul(a,3)^b^c^gmul(d,2);
    }
}

static void aes128_encrypt_block(const uint8_t key[16], const uint8_t in[16],
                                   uint8_t out[16]) {
    uint8_t s[16]; memcpy(s, in, 16);
    /* XOR with round key 0 */
    for (int i=0;i<16;i++) s[i]^=key[i];

    /* 10 rounds - simplified (no proper key schedule) */
    for (int r=0;r<9;r++) {
        aes_sub_bytes(s);
        aes_shift_rows(s);
        aes_mix_columns(s);
        for (int i=0;i<16;i++) s[i]^=key[i]; /* simplified */
    }
    aes_sub_bytes(s);
    aes_shift_rows(s);
    for (int i=0;i<16;i++) s[i]^=key[i];
    memcpy(out, s, 16);
}

/* CTR mode encryption (AES-128-CTR, basis for GCM) */
int fg_aes128_ctr_xor(const uint8_t key[16], const uint8_t iv[16],
                       uint8_t* out, const uint8_t* in, size_t len) {
    if (!key || !iv || !out || !in) return FG_ERR_INVAL;

    uint8_t counter[16]; memcpy(counter, iv, 16);
    uint8_t keystream[16];
    size_t pos = 0;

    while (pos < len) {
        aes128_encrypt_block(key, counter, keystream);
        size_t n = (len - pos > 16) ? 16 : len - pos;
        for (size_t i=0;i<n;i++) out[pos+i] = in[pos+i] ^ keystream[i];
        pos += n;
        /* increment counter (big-endian) */
        for (int i=15;i>=0;i--) { if (++counter[i]) break; }
    }
    return FG_OK;
}
