...
int fg_hmac_sha256(const uint8_t* key,  size_t key_len,
    ...
    if (!key || !data || !out) return FG_ERR_INVAL;
...
int fg_hkdf_expand(const uint8_t* prk,  size_t prk_len,
    ...
    if (!prk || !okm) return FG_ERR_INVAL;
    if (okm_len > 255 * 32) return FG_ERR_INVAL;
    ...
    while (done < okm_len) {
        ...
        size_t  buf_len = 0;
        if (t_len) { memcpy(buf, t, 32); buf_len=32; }
        if (info && info_len) { memcpy(buf+buf_len, info, info_len); buf_len+=info_len; }
        buf[buf_len++] = counter++;

        int rc = fg_hmac_sha256(prk, prk_len, buf, buf_len, t);
